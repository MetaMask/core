...
In the [previous section](./01-getting-started.mdx), we started by creating a data service class and a messenger to go along with it. Here's what we've got so far:

<details>
  <summary>View code</summary>
  <CodeBlock language="typescript" title="packages/orders-service/src/orders-service.ts">{ordersService}</CodeBlock>
</details>
...
Continue to [**Part 3: State-Mutating Requests**](./03-making-state-mutating-requests.mdx) →
