...
We want to ensure that the data we get back from the server is in a format we expect. We _could_ simply typecast the response data, but instead, we use the [Superstruct](https://docs.superstructjs.org/) library to define a schema (using predefined types in `@metamask/utils`), and we match the data against it. As a plus, we can use the schema to define the `FetchOrdersResponse` type:
