...
Let's start with the `queryKey` option. There are some rules about what we put in this array:

- The first item must be the name of the messenger action that we will register this method under (including the namespace). Here we use `OrdersService:fetchOrders` (or `${this.name}:fetchOrders` for short).
- As for the remaining items, we want requests with different query parameters to be cached differently, and we can do this by passing the same query parameters that we pass to the URL:
...
1. We check the HTTP status of the response and throw error if it is not within the 200-299 range. We use a special error, `HttpError`, which comes from `@metamask/controller-utils`. It's important that we use this instead of a plain `Error` because it informs `fetchQuery` which kinds of HTTP responses to retry.
