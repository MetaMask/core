...
- `DeFiPositionsControllerV2` reads processing-poll `maxAttempts` / `pollInterval` from the `defiControllerV2` remote feature flag via `RemoteFeatureFlagController:getState` (clients must delegate that action); missing or invalid values fall back to 3 attempts and a 3000ms poll interval
