...
> {
   ...
   */
  private get selectedAccounts(): InternalAccount[] {
   ...
   */
  private readonly dataSources: Map<string, Set<ChainId>> = new Map();
   ...
   */
  private subscribeToDataSources(): void {
    if (this.selectedAccounts.length === 0) {
      return;
    }
   ...
   */
  private subscribeAssetsBalance(): void {
   ...
   *
   * @param accounts
   * @param chainsToSubscribe
   */
  private buildChainToAccountsMap(
    accounts: InternalAccount[],
   ...
   *
   * @param chains
   * @param chainToAccounts
   */
  private getAccountsForChains(
    chains: ChainId[],
   ...
   * @param sourceId
   */
  private unsubscribeDataSource(sourceId: string): void {
    const subscriptionKey = `ds:${sourceId}`;
    const existingSubscription = this.activeSubscriptions.get(subscriptionKey);
