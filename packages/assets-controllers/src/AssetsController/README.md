    ...
    └── For each selectedAccount:
        ...
        └── getAssets([account], { chainIds, forceUpdate: true })
            ...
            └── executeMiddlewares(request)
                ...
                ├── DetectionMiddleware marks new assets
...
└── handleSubscriptionUpdate(response)
    ...
    │   ├── DetectionMiddleware - Marks newly detected assets
    │   ├── TokenDataSource - Fetches missing metadata
    │   └── PriceDataSource - Fetches prices for new assets
    │
    └── updateState(enrichedResponse)
        ...
        └── Publish events:
            ...
            └── AssetsController:assetsDetected (if new assets)
```
...
Emitted when new assets are discovered for an account.
...
|------------|---------|--------------|
| DetectionMiddleware | Identifies newly discovered assets | After balance fetch |
