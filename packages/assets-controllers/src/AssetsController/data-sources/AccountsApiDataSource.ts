...
function buildAssetId(
  ...
  return `${chainId}/erc20:${checksummedAddress}` as Caip19AssetId;
}
...
> {
  ...
  async #initializeActiveChains(): Promise<void> {
    ...
    try {
      ...
      this.updateActiveChains(chains, (c) => {
        this.#messenger.call('AssetsController:activeChainsUpdate', CONTROLLER_NAME, c);
  ...
  async fetch(request: DataRequest): Promise<DataResponse> {
    ...
    try {
      ...
      const caipAddresses = addresses.flatMap((address) =>
      ...
      // Handle unprocessed networks - these should be treated as errors
      // so lower-priority data sources can try to handle them
      if (apiResponse.unprocessedNetworks.length > 0) {
        const unprocessedChainIds =
          apiResponse.unprocessedNetworks.map(decimalToChainId);
        log('Unprocessed networks detected (will fallback to other data sources)', {
          unprocessedChainIds,
        });

        // Add unprocessed chains to errors so middleware keeps them for fallback
        response.errors = {};
      ...
      log('Fetch SUCCESS', {
        ...
        failedChains: Object.keys(response.errors ?? {}).length,
      });
