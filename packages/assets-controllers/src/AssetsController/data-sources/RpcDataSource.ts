...
export type RpcDataSourceActiveChainsChangedEvent = {
  type: 'RpcDataSource:activeChainsChanged';
  payload: [ChainId[]];
};
...
function getChainNativeTokenInfo(chainId: ChainId): {
  symbol: string;
  name: string;
} {
  const reference = chainId.split(':')[1];

  // Map common chains to their native tokens
  const nativeTokens: Record<string, { symbol: string; name: string }> = {
    ...
    '59144': { symbol: 'ETH', name: 'Linea' },
  };

  return nativeTokens[reference] ?? { symbol: 'ETH', name: 'Ether' };
}
...
> {
  ...
  #updateFromNetworkState(networkState: NetworkState): void {
    ...
    // Update active chains and publish if changed
    this.updateActiveChains(activeChains, (chains) =>
      this.#messenger.publish('RpcDataSource:activeChainsChanged', chains),
    );
  }
  ...
  async fetch(request: DataFetchRequest): Promise<DataResponse> {
    ...
    for (const chainId of chainsToFetch) {
      ...
      // Get token info from chain status or fallback to static mapping
      const chainStatus = this.state.chainStatuses[chainId];
      const tokenInfo = chainStatus
        ? { symbol: chainStatus.nativeCurrency, name: chainStatus.name }
        : getChainNativeTokenInfo(chainId);
    ...
    if (failedChains.length > 0) {
      ...
      log('Fetch SUCCESS', {
        ...
        accountCount: Object.keys(assetsBalance).length,
        assetCount: Object.keys(assetsMetadata).length,
      });
    }
