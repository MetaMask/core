...
export type BackendWebsocketDataSourceFetchAction = {
  type: 'BackendWebsocketDataSource:fetch';
...
export type BackendWebsocketDataSourceActions =
  ...
  | BackendWebsocketDataSourceFetchAction
...
export type BackendWebsocketDataSourceActiveChainsChangedEvent = {
  type: 'BackendWebsocketDataSource:activeChainsChanged';
  payload: [ChainId[]];
};
...
export interface BackendWebsocketDataSourceOptions {
  messenger: BackendWebsocketDataSourceMessenger;
  /** Chains that this data source supports (from AccountsApiDataSource) */
...
> {
  ...
  constructor(options: BackendWebsocketDataSourceOptions) {
    ...
    // Initialize with supported chains if provided
    if (options.supportedChains) {
      this.state.activeChains = [...options.supportedChains];
    }

    log('Initializing BackendWebsocketDataSource', {
  ...
  #registerActionHandlers(): void {
    ...
    const fetchHandler: BackendWebsocketDataSourceFetchAction['handler'] =
      async (request) => this.fetch(request);
  ...
  #subscribeToEvents(): void {
    ...
    this.#messenger.subscribe(
      ...
      (connectionInfo) => {
        ...
        if (connectionInfo.state === ('connected' as WebSocketState)) {
          ...
          log('WebSocket disconnected, clearing subscriptions');
    ...
    this.#messenger.subscribe(
      ...
      (chains: ChainId[]) => {
        log('Active chains changed from AccountsApiDataSource', { chains });
  ...
  async #processPendingSubscriptions(): Promise<void> {
    ...
    log('Processing pending subscriptions', {
      count: this.#pendingSubscriptions.size,
      subscriptionIds: Array.from(this.#pendingSubscriptions.keys()),
    });
    ...
    for (const [subscriptionId, request] of pendingEntries) {
      try {
        ...
        log('Pending subscription processed', { subscriptionId });
      } catch (error) {
  ...
  // ============================================================================
  // FETCH (One-time data retrieval - not supported for WebSocket-only source)
  // ============================================================================

  /**
   * Fetch is not supported for this data source as it's WebSocket-only.
   * Use subscribe() for real-time updates.
   */
  async fetch(_request: DataFetchRequest): Promise<DataResponse> {
    // WebSocket data source doesn't support one-time fetch
    // Return empty response - use AccountsApiDataSource for initial data
    return {};
  }
  ...
  async subscribe(subscriptionRequest: SubscriptionRequest): Promise<void> {
    ...
    try {
      ...
      if (connectionInfo.state !== ('connected' as WebSocketState)) {
        log('WebSocket not connected, queuing subscription for later', {
          state: connectionInfo.state,
          subscriptionId,
        });
      ...
      log('Could not get connection info, queuing subscription', {
        error,
        subscriptionId,
      });
    ...
    log('Creating WebSocket subscription', {
      subscriptionId,
      channels,
      namespaces,
    });
    ...
    try {
      ...
      this.activeSubscriptions.set(subscriptionId, {
        cleanup: async () => {
          log('Cleaning up WebSocket subscription', { subscriptionId });
          const wsSub = this.#wsSubscriptions.get(subscriptionId);
          if (wsSub) {
            try {
              await wsSub.unsubscribe();
            } catch (error) {
              log('Error unsubscribing', { subscriptionId, error });
            }
      ...
      log('WebSocket subscription SUCCESS', {
        subscriptionId,
        channels,
        chains: chainsToSubscribe,
      });
  ...
  #handleNotification(
    ...
    try {
      ...
      if (accountIndex === -1) {
        log('Address not found in request', { address });
        return;
      }
      ...
      if (Object.keys(response).length > 0) {
        log('Publishing balance update', {
          accountId,
          chainId,
          assetCount: Object.keys(response.assetsMetadata ?? {}).length,
        });
        // Publish update
        this.#messenger.publish(
          'BackendWebsocketDataSource:assetsUpdated',
  ...
  destroy(): void {
    log('Destroying BackendWebsocketDataSource', {
      subscriptionCount: this.#wsSubscriptions.size,
    });
