import type { AssetByIdResponse } from '@metamask/core-backend';
...
export class TokenDataSource {
  ...
  async #getSupportedNetworks(): Promise<Set<string>> {
    ...
    try {
      ...
      const response = await this.#apiClient.fetchTokenV2SupportedNetworks();
    ...
    return forDataTypes(['metadata'], async (ctx, next) => {
      ...
      try {
        ...
        const metadataResponse =
          await this.#apiClient.fetchV3Assets(supportedAssetIds);
        ...
        for (const assetData of metadataResponse) {
          ...
          response.assetsMetadata[caipAssetId] =
            transformAssetByIdResponseToMetadata(assetData.assetId, assetData);
        }
      } catch (error) {
