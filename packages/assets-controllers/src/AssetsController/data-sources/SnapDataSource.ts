...
import type { ChainId, Caip19AssetId, DataRequest, DataResponse, Middleware } from '../types';
import {
  AbstractDataSource,
...
> {
  ...
  #subscribeToSnapKeyringEvents(): void {
    ...
    try {
      ...
      log('Subscribed to AccountsController:accountBalancesUpdated');
    } catch (error) {
   ...
   */
  #handleSnapBalancesUpdated(payload: AccountBalancesUpdatedEventPayload): void {
    log('Received snap balance update', {
  ...
  addNetworks(chainIds: ChainId[]): void {
    ...
    if (newChains.length > 0) {
      ...
      this.updateActiveChains(updated, (c) =>
        this.#messenger.call('AssetsController:activeChainsUpdate', SNAP_DATA_SOURCE_NAME, c),
      );
  ...
  async fetch(request: DataRequest): Promise<DataResponse> {
    ...
    log('Fetch completed', {
      ...
      failedChains: Object.keys(mergedResponse.errors ?? {}).length,
    });
  ...
  /**
   * Get the middleware for fetching balances via Snap snaps.
   * This middleware:
   * - Filters request to only chains this data source supports
   * - Fetches balances for those chains via installed snaps
  ...
  async subscribe(subscriptionRequest: SubscriptionRequest): Promise<void> {
    ...
    log('Subscription SUCCESS (event-based, no polling)', {
      subscriptionId,
