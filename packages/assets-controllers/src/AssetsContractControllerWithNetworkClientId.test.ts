...
describe('AssetsContractController with NetworkClientId', () => {
  ...
  it('should not have balance in a single call after switching to network without token detection support', async () => {
    ...
    // Mock getTokenBalancesForMultipleAddresses to return a balance for mainnet
    const mockBalance = new BN('0733ed8ef4c4a0155d09', 16);
    jest
      .spyOn(multicallUtils, 'getTokenBalancesForMultipleAddresses')
    ...
    // Sepolia also has Multicall3 support, so it should also return balances
    const sepoliaBalances = await messenger.call(
      `AssetsContractController:getBalancesInSingleCall`,
      ERC20_SAI_ADDRESS,
      [ERC20_SAI_ADDRESS],
      'sepolia',
    );
    expect(sepoliaBalances[ERC20_SAI_ADDRESS]).toBeDefined();
    messenger.clearEventSubscriptions('NetworkController:networkDidChange');
  });
