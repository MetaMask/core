...
describe('TokenDetectionController', () => {
  ...
  describe('constructor', () => {
    describe('selectedAccountId', () => {
      it('should get the selectedAccount from accounts controller if not defined in constructor arguments', async () => {
        await withController(
          {
            options: {},
          },
          async ({ mockGetAccount, callActionSpy }) => {
      ...
      it('should if not start non evm account was passed and selectedAccountId defined in constructor arguments', async () => {
        const nonEvmAccount = createMockInternalAccount({
          // @ts-expect-error mocking a bitcoin account
          type: 'bitcoin',
        });
        await withController(
          {
          ...
          async ({ controller, mockGetAccount, callActionSpy }) => {
            mockGetAccount(nonEvmAccount);
            expect(callActionSpy).toHaveBeenCalledWith(
              'AccountsController:getSelectedAccount',
            );
  ...
  describe('AccountsController:selectedAccountChange', () => {
    ...
    it('should not restart detection for nonevm accounts', async () => {
      const mockGetBalancesInSingleCall = jest.fn().mockResolvedValue({
        [sampleTokenA.address]: new BN(1),
      });
      const nonEvmAccount = createMockInternalAccount({
  ...
  describe('detectTokens', () => {
    ...
    it('should not detect tokens internal account returned from getAccountExpect is nonevm', async () => {
      const mockGetBalancesInSingleCall = jest.fn().mockResolvedValue({
        [sampleTokenA.address]: new BN(1),
      });
      const nonEvmAccount = createMockInternalAccount({
      ...
      await withController(
        {
          options: {
            disabled: false,
            getBalancesInSingleCall: mockGetBalancesInSingleCall,
            selectedAccountId: nonEvmAccount.id,
          },
        },
        async ({
          controller,
          ...
          await controller.detectTokens({
            networkClientId: NetworkType.goerli,
            selectedAddress: nonEvmAccount.address,
          });
          expect(callActionSpy).not.toHaveBeenCalledWith(
            'TokensController:addDetectedTokens',
          );
