...
> {
  ...
  // Deprecated to improve performance since there isno need to call this endpoint so many times to get 3 fields (collection.creator, collection.contractDeployedAt and collection.topBid)
  // This is a helper function to get the contract info for an nft and append it to the token.collection
  async getContractInfoForNft(apiNfts: TokensResponse[]) {
    ...
    if (Object.values(collections).some((contracts) => contracts.length > 0)) {
      ...
      collectionsResponses.forEach((singleCollectionResponse) => {
        if ((singleCollectionResponse as GetCollectionsResponse)?.collections) {
          ...
          if (found) {
            singleNFT.token = {
              ...
              collection: {
                ...
                openseaVerificationStatus: found?.openseaVerificationStatus, // Deprecated
                contractDeployedAt: found.contractDeployedAt,
                creator: found?.creator,
                ownerCount: found.ownerCount, // Deprecated
                topBid: found.topBid,
              },
            };
          }
