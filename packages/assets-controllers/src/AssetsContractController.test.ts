...
describe('AssetsContractController', () => {
  ...
  it('should get balance of ERC-20 token in a single call on network with token detection support', async () => {
    ...
    // Mock Multicall3 aggregate3 call
    const mockBalance = '0x0733ed8ef4c4a0155d09';

    // Mock the provider's sendAsync method to intercept eth_call
    const mockResponse = defaultAbiCoder.encode(
      ['tuple(bool success, bytes returnData)[]'],
      [[[true, defaultAbiCoder.encode(['uint256'], [mockBalance])]]],
    );
  ...
  it('should track and use the currently selected chain ID and provider when getting balances in a single call', async () => {
    ...
    const mockBalanceLinea = '0xa0155d09733ed8ef4c4';
    ...
    // Track current network balance
    let currentBalance = mockBalanceMainnet;
    ...
    const sendAsyncSpy = jest
      ...
      .mockImplementation((req, callback) => {
        if (req.method === 'eth_call') {
          const mockResponse = defaultAbiCoder.encode(
            ['tuple(bool success, bytes returnData)[]'],
            [[[true, defaultAbiCoder.encode(['uint256'], [currentBalance])]]],
          );
          return callback(null, { result: mockResponse });
        }
        if (req.method === 'eth_chainId') {
          return callback(null, { result: ChainId.mainnet });
        }
        if (req.method === 'eth_blockNumber') {
          return callback(null, { result: '0x3b3301' });
        }
        // For unexpected methods, return an empty result
        return callback(null, { result: null });
      });
    ...
    // Switch balance for "linea-mainnet"
    currentBalance = mockBalanceLinea;
    ...
    expect(balancesOnLineaMainnet[ERC20_SAI_ADDRESS].toString(16)).toBe(
      'a0155d09733ed8ef4c4',
    );
  ...
  it('should not have balance in a single call after switching to network without token detection support', async () => {
    ...
    // Mock Multicall3 aggregate3 call for mainnet with balance
    const mockBalanceMainnet = '0x0733ed8ef4c4a0155d09';
    // Mock zero balance for "Sepolia"
    const mockBalanceSepolia = '0x0';
    ...
    // Switch to "Sepolia" by changing balance to 0
    currentBalance = mockBalanceSepolia;
    ...
    // With Multicall3, Sepolia is now supported, but returns 0 balance (empty result)
