...
import { reportError, safe } from './errors.js';
...
export class SnapAccountService {
  ...
  #handleUnlock(): void {
    // eslint-disable-next-line no-void
    void safe('Snap keyring v2 migration', () => this.ensureMigrated()).then(
      async () => {
    ...
    await this.#messenger.call(
      ...
      async (controller) => {
        ...
        for (const state of states.values()) {
          ...
          await safe(`Adding v2 Snap keyring for "${state.snapId}"`, async () =>
        ...
        await safe('Removing legacy Snap keyring', async () =>
