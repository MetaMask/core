...
describe('wallet_revokeSession', () => {
  ...
  it('partially revokes the CAIP-25 endowment permission if `sessionScopes` param is passed in', async () => {
    ...
    await handler({ ...baseRequest, params: { sessionScopes: ['eip155:1'] } });
  ...
  it('not call `updateCaveat` if `sessionScopes` param is passed in with non existing permitted scope', async () => {
    const { handler, getCaveatForOrigin, updateCaveat } = createMockedHandler();
    getCaveatForOrigin.mockImplementation(() => ({
      value: {
        optionalScopes: {
    ...
    await handler({ ...baseRequest, params: { sessionScopes: ['eip155:5'] } });
