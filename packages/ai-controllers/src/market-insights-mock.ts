import type { MarketInsightsReport } from './ai-digest-types';

/**
 * Mock market insights data for BTC.
 * TODO: Remove this file once the API is live.
 */
...
/**
 * Returns mock market insights for a given CAIP-19 asset identifier.
 * Currently only BTC-like identifiers are supported.
 * TODO: Remove once the API is live.
 *
 * @param caip19Id - The CAIP-19 identifier.
 * @returns The mock report, or `null` if no mock exists for the asset.
 */
export function getMockMarketInsights(
  caip19Id: string,
): MarketInsightsReport | null {
  // Match any BTC CAIP-19 identifier (e.g. "eip155:1/slip44:0", or anything containing "btc")
  const id = caip19Id.toLowerCase();
  if (id.includes('btc') || id.includes('slip44:0')) {
