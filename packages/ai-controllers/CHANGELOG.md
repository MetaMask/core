...
- Validate `searchDigest` API responses and throw when the payload does not match the expected `MarketInsightsReport` shape ([#7930](https://github.com/MetaMask/core/pull/7930))
- Normalize `searchDigest` responses from either direct report payloads or `digest` envelope payloads ([#7930](https://github.com/MetaMask/core/pull/7930))

### Removed

- Remove legacy digest APIs and digest cache from `AiDigestController` and `AiDigestService`; only market insights APIs remain ([#7930](https://github.com/MetaMask/core/pull/7930))
