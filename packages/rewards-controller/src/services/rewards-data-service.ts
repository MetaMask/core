...
// Function to get subscription token
type GetSubscriptionToken = (subscriptionId: string) => Promise<TokenResponse>;
...
export class RewardsDataService {
  ...
  constructor({
    ...
    this.#locale = locale;
    this.#rewardsApiUrl = rewardsApiUrl;
    this.#environment = environment;
    this.#getSubscriptionToken = getSubscriptionToken;
  ...
  private async makeRequest(
    ...
    headers['rewards-client-id'] = `${this.#appType}-${this.#version}`;

    // Add bearer token for authenticated requests
    try {
      if (subscriptionId) {
        const tokenResult = await this.#getSubscriptionToken(subscriptionId);
  ...
  async fetchGeoLocation(): Promise<string> {
    ...
    try {
      const response = await successfulFetch(
        GEOLOCATION_URLS[this.#environment],
      );
      ...
      log('RewardsDataService: Failed to fetch geoloaction', e);
      return location;
    }
  }
