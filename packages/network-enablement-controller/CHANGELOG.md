...
- Add `listPopularNetworks()` method and `NetworkEnablementController:listPopularNetworks` messenger action ([#8105](https://github.com/MetaMask/core/pull/8105))
  - Returns CAIP-2 chain IDs for popular EVM networks (from POPULAR_NETWORKS) and Bitcoin, Solana, and Tron mainnets that are present in NetworkController `networkConfigurationsByChainId` and MultichainNetworkController `multichainNetworkConfigurationsByChainId` respectively.
