...
export type FetchBalanceWithFallbackOptions = {
   ...
   *
   * Implies a cache-bypassing `fetchPositions({ fresh: true })` read so a
   * rejected body cannot seed the steady-state client cache.
   */
  minBlock?: number;
  /**
   ...
   * `minBlock`, but `minBlock` already implies the same cache-bypass path.
   */
  fresh?: boolean;
};
