  ...
  describe('Transak WebSocket integration', () => {
    it('subscribes to WS channel when a pending Transak order is added', async () => {
      await withController(async ({ controller, rootMessenger }) => {
        ...
        const order = createMockOrder({
          id: '/providers/transak-native/orders/ws-order-1',
          providerOrderId: 'ws-order-1',
          status: RampsOrderStatus.Pending,
          provider: { id: 'transak-native', name: 'Transak' },
        });
    ...
    it('subscribes to WS for legacy /providers/transak-native provider ids', async () => {
      await withController(async ({ controller, rootMessenger }) => {
        const subscribeHandler = jest.fn();
        rootMessenger.registerActionHandler(
          'TransakService:subscribeToOrder',
        ...
        const order = createMockOrder({
          id: '/providers/transak-native/orders/ws-order-legacy',
          providerOrderId: 'ws-order-legacy',
          status: RampsOrderStatus.Pending,
          provider: { id: '/providers/transak-native', name: 'Transak' },
        });
        controller.addOrder(order);

        expect(subscribeHandler).toHaveBeenCalledWith('ws-order-legacy');
      });
    });

    it('subscribes to WS for transak-native-staging provider ids', async () => {
      await withController(async ({ controller, rootMessenger }) => {
        const subscribeHandler = jest.fn();
        rootMessenger.registerActionHandler(
          'TransakService:subscribeToOrder',
        ...
        const order = createMockOrder({
          id: '/providers/transak-native-staging/orders/ws-order-staging',
          providerOrderId: 'ws-order-staging',
          status: RampsOrderStatus.Pending,
          provider: { id: 'transak-native-staging', name: 'Transak' },
        });
        controller.addOrder(order);

        expect(subscribeHandler).toHaveBeenCalledWith('ws-order-staging');
      });
    });
    ...
    it('does not subscribe to WS for completed orders', async () => {
      await withController(async ({ controller, rootMessenger }) => {
        ...
        const order = createMockOrder({
          id: '/providers/transak-native/orders/ws-completed',
    ...
    it('does not subscribe to WS for non-Transak orders', async () => {
      await withController(async ({ controller, rootMessenger }) => {
        ...
        const order = createMockOrder({
          id: '/providers/moonpay/orders/moonpay-order-1',
    ...
    it('unsubscribes from WS when a Transak order is removed', async () => {
      await withController(async ({ controller, rootMessenger }) => {
        ...
        const order = createMockOrder({
          id: '/providers/transak-native/orders/ws-remove-1',
    ...
    it('does not refresh for unrecognized order IDs', async () => {
      await withController(async ({ controller, rootMessenger }) => {
        const nonTransakOrder = createMockOrder({
          id: '/providers/moonpay/orders/moonpay-order',
    ...
    it('does not refresh when event is for a non-Transak order', async () => {
      await withController(async ({ controller, rootMessenger }) => {
        ...
        const moonpayOrder = createMockOrder({
          id: '/providers/moonpay/orders/moonpay-123',
    ...
    it('handles errors in refreshOrder silently when triggered by orderUpdate event', async () => {
      await withController(async ({ controller, rootMessenger }) => {
        ...
        const order = createMockOrder({
          id: '/providers/transak-native/orders/error-order',
    ...
    it('subscribeToTransakOrderUpdates subscribes all pending Transak orders', async () => {
      await withController(async ({ controller, rootMessenger }) => {
        ...
        const pendingOrder = createMockOrder({
          id: '/providers/transak-native/orders/bulk-1',
        ...
        const completedOrder = createMockOrder({
          id: '/providers/transak-native/orders/bulk-2',
        ...
        const moonpayOrder = createMockOrder({
          id: '/providers/moonpay/orders/bulk-3',
    ...
    it('exposes subscribeToTransakOrderUpdates via messenger', async () => {
      await withController(async ({ controller, rootMessenger }) => {
        const subscribeHandler = jest.fn();
        rootMessenger.registerActionHandler(
          'TransakService:subscribeToOrder',
        ...
        controller.addOrder(
          createMockOrder({
            id: '/providers/transak-native/orders/messenger-ws-1',
            providerOrderId: 'messenger-ws-1',
            status: RampsOrderStatus.Pending,
            provider: { id: 'transak-native', name: 'Transak' },
          }),
        );
        ...
        rootMessenger.call('RampsController:subscribeToTransakOrderUpdates');

        expect(subscribeHandler).toHaveBeenCalledWith('messenger-ws-1');
      });
    });
