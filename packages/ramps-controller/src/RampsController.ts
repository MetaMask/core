...
> {
  ...
  async init(options?: ExecuteRequestOptions): Promise<void> {
    ...
    console.log('****** RAMP CONTROLLER INIT userRegion ******', userRegion);

    if (!userRegion) {
      console.log('****** RAMP CONTROLLER INIT getGeolocation ******');
      const regionCode = await this.messenger.call(
        'RampsService:getGeolocation',
      );
      console.log('****** RAMP CONTROLLER INIT regionCode ******', regionCode);
      if (!regionCode) {
        throw new Error(
      ...
      const countries = await this.getCountries('buy', options);
      console.log('****** RAMP CONTROLLER INIT countries ******', countries);
      userRegion = findRegionFromCode(regionCode, countries);

      if (!userRegion) {
        throw new Error(
          `Region "${regionCode}" not found in countries data. Cannot initialize controller.`,
        );
      }
    ...
    console.log('****** RAMP CONTROLLER INIT countries ******', this.state.countries);

    if (!this.state.countries || this.state.countries.length === 0) {
      this.triggerGetCountries('buy', options);
    }
  ...
  async getCountries(
    ...
    try { 
       countries = await this.executeRequest(
        cacheKey,
        async () => {
      ...
      console.error('****** RAMP CONTROLLER GET COUNTRIES ERROR ******', error);
      throw error;
    }
