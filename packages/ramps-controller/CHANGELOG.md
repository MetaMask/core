...
- Resolve the vendor customer identity for autoramps via a locally declared `KycController:getCustomerIdentity` messenger action type instead of a package dependency on `@metamask/kyc-controller`, keeping the ramps↔kyc packages decoupled in the monorepo build.
