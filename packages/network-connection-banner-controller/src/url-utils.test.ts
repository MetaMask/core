...
describe('getDomain', () => {
  ...
  it('falls back to the hostname when psl cannot resolve it (e.g., .local suffix)', () => {
    // `.local` is reserved but not part of the Public Suffix List, so
    // `psl.get` returns null and getDomain returns the hostname verbatim.
    expect(getDomain('http://foo.local/')).toBe('foo.local');
  });
});
