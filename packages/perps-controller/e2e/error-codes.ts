...
async function main(): Promise<void> {
  ...
  for (const [key, value] of codes) {
    runner.assert(`${key} is string`, typeof value === 'string', `got ${typeof value}`);
  ...
  runner.assertGt('FallbackMaxLeverage > 0', MARGIN_ADJUSTMENT_CONFIG.FallbackMaxLeverage, 0);
  runner.assert('FallbackMaxLeverage <= 200', MARGIN_ADJUSTMENT_CONFIG.FallbackMaxLeverage <= 200,
