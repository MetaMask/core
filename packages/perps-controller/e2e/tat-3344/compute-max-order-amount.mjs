...
// recipe uses.
const harnessBin = process.env.MM_HARNESS_BIN ?? process.env.METAMASK_HARNESS_BIN;
if (!harnessBin) {
...
const { getMaxAllowedAmount } = await import(
  pathToFileURL(path.join(projectRoot, 'packages/perps-controller/src/index.ts'))
    .href
);

const input = { action: 'tat-3344.compute-max', context: { projectRoot }, node: { network } };
...
const prefixMax = Math.max(
  0,
  Math.floor(Math.floor(spendableBalance * leverage) * (1 - MAX_ORDER_MARGIN_BUFFER)),
);
...
const record = {
  ...
  computedVia: 'packages/perps-controller/src/utils/orderCalculations.ts#getMaxAllowedAmount',
};

process.stderr.write(`[tat-3344] ${JSON.stringify(record)}\n`);
if (outPath) {
  await writeFile(path.resolve(outPath), `${JSON.stringify(record, null, 2)}\n`, 'utf-8');
}
