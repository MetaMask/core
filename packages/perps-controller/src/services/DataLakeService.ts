...
// API uses snake_case: sl_price, tp_price - kept for API compatibility
...
export class DataLakeService {
  ...
  async reportOrder(options: {
    ...
    this.#deps.debugLogger.log('DataLake API: Starting order report', {
      ...
      hasStopLoss: Boolean(sl_price),
      hasTakeProfit: Boolean(tp_price),
    ...
    try {
      const token =
        await this.#deps.controllers.authentication.getBearerToken();
      const evmAccount =
      ...
      if (retryCount < MAX_RETRIES) {
        ...
        setTimeout(() => {
          this.reportOrder({
            ...
            this.#deps.logger.error(ensureError(retryError), {
              context: {
                name: 'DataLakeService.reportOrder',
                data: {
                  operation: 'retry',
