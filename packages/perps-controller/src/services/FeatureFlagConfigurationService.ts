/* eslint-disable id-denylist, no-restricted-syntax, no-negated-condition */
...
/**
 * Remote feature flags state shape for perps-related flags.
 * This is a minimal type definition to avoid direct dependency on
 * RemoteFeatureFlagController which can cause build issues.
 */
...
/**
 * Version gated feature flag structure for validating remote flags.
 */
...
/**
 * Check if a value is a valid version gated feature flag.
 *
 * @param flag - The value to check
 * @returns True if the value has a boolean 'value' property
 */
function isVersionGatedFeatureFlag(
  flag: unknown,
): flag is VersionGatedFeatureFlag {
  return (
    typeof flag === 'object' &&
    flag !== null &&
    'value' in flag &&
    typeof (flag as VersionGatedFeatureFlag).value === 'boolean'
  );
}

/**
 * Extract the boolean value from a version gated feature flag.
 *
 * @param flag - The version gated feature flag
 * @returns The boolean value or undefined if invalid
 */
function validatedVersionGatedFeatureFlag(
  flag: VersionGatedFeatureFlag,
): boolean | undefined {
  return flag.value;
}
...
export class FeatureFlagConfigurationService {
   ...
   *
   * @param remoteValue - The raw value from remote feature flags
   ...
   * @returns True if arrays have different values
   */
  ...
  refreshHip3Config(options: {
    remoteFeatureFlagControllerState: RemoteFeatureFlagState;
  ...
  setBlockedRegions(options: {
    ...
    context.refreshEligibility().catch((err) => {
