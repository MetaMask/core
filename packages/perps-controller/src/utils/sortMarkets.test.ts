...
import type { SortDirection, SortField } from './sortMarkets';
...
describe('sortMarkets', () => {
  ...
  describe('volume sorting', () => {
    ...
    it('places markets with undefined volume at the end', () => {
      ...
      // With real parseVolume, undefined returns 0, so ETH goes to end
      expect(result[2].symbol).toBe('ETH');
    });
  ...
  describe('edge cases', () => {
    ...
    it('does not mutate original array', () => {
      ...
      const originalOrder = markets.map((market) => market.symbol);
