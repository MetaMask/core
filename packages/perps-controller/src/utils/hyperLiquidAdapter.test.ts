...
import type { RawHyperLiquidLedgerUpdate } from '../types';
...
describe('hyperLiquidAdapter', () => {
  describe('adaptOrderToSDK', () => {
    ...
    it('should handle missing price for limit order', () => {
      ...
      expect(result.t).toStrictEqual({ limit: { tif: 'Gtc' } });
    });
    ...
    it('should throw error for unknown coin', () => {
      ...
      expect(() => adaptOrderToSDK(order, symbolToAssetId)).toThrow(
        /Asset UNKNOWN not found in asset mapping/u,
      );
    });
  });
  ...
  describe('adaptOrderFromSDK', () => {
    ...
    it('should handle market order', () => {
      ...
      expect(result).toStrictEqual({
        ...
        size: '10',
        originalSize: '10',
        price: '', // Empty limitPx is passed through as-is (nullish coalescing doesn't catch empty strings)
        filledSize: '0',
        remainingSize: '10',
        status: 'open',
        timestamp: 1234567890000,
    ...
    it('should handle trigger order with triggerPx', () => {
      ...
      expect(result).toStrictEqual({
        ...
        size: '5',
        originalSize: '5',
        price: '', // Empty limitPx is passed through (nullish coalescing doesn't catch empty strings)
        filledSize: '0',
        remainingSize: '5',
        status: 'open',
        timestamp: 1234567890000,
    ...
    it('converts child order with limitPx when triggerPx is null/undefined', () => {
      // Test the case where triggerPx is undefined (nullish coalescing falls back to limitPx)
      const frontendOrder: FrontendOrder = {
        oid: 66666,
        coin: 'ADA',
        side: 'B',
        ...
        children: [
          {
            ...
            sz: '1000',
            origSz: '1000',
            limitPx: '0.6', // Using limitPx as fallback
            orderType: 'Take Profit Limit',
            timestamp: 1234567890001,
            isTrigger: true,
            reduceOnly: true,
            triggerCondition: '',
            // Undefined triggerPx - nullish coalescing falls back to limitPx
            children: [],
            isPositionTpsl: true,
            tif: null,
            cloid: null,
      ...
      } as unknown as FrontendOrder; // Override to simulate SDK response without triggerPx

      const result = adaptOrderFromSDK(frontendOrder);

      // Nullish coalescing falls back to limitPx when triggerPx is undefined
    ...
    it('sets triggerPrice for trigger orders with only triggerPx (market style)', () => {
      ...
      // price uses empty limitPx (nullish coalescing doesn't catch empty strings)
  ...
  describe('adaptPositionFromSDK', () => {
    it('should convert asset position correctly', () => {
      ...
      expect(result).toStrictEqual({
        ...
        unrealizedPnl: '2500',
        marginUsed: '25000',
        leverage: { type: 'cross', value: 3, rawUsd: undefined },
