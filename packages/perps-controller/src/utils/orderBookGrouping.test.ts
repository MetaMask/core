...
describe('orderBookGrouping', () => {
  describe('calculateGroupingOptions', () => {
    ...
    it('handles zero price with fallback', () => {
      ...
      expect(options).toStrictEqual([0.01, 0.1, 1]);
    });
  ...
  describe('calculateAggregationParams', () => {
    describe('guard clause for invalid inputs', () => {
      it('returns nSigFigs: 5 for price <= 0', () => {
        ...
        expect(calculateAggregationParams(10, -100)).toStrictEqual({
          nSigFigs: 5,
        });
      });

      it('returns nSigFigs: 5 for grouping <= 0', () => {
        expect(calculateAggregationParams(0, 50000)).toStrictEqual({
          nSigFigs: 5,
        });
        expect(calculateAggregationParams(-10, 50000)).toStrictEqual({
          nSigFigs: 5,
        });
      });
    });
    ...
    describe('BTC at ~$90,000', () => {
      ...
      it('returns nSigFigs: 5 with mantissa: 2 for grouping 1', () => {
        ...
        expect(result).toStrictEqual({ nSigFigs: 5, mantissa: 2 });
      });
