...
describe('orderUtils', () => {
  describe('formatOrderLabel', () => {
    ...
    it('should handle trigger orders - isTrigger is fallback for reduceOnly', () => {
      // When reduceOnly is explicitly false, isTrigger is NOT used as a fallback
      // This is because: Boolean(reduceOnly ?? isTrigger) = Boolean(false ?? true) = Boolean(false) = false
      const order: Order = {
        orderId: '1',
        symbol: 'ETH',
        side: 'buy',
      ...
      // With reduceOnly=false, it's treated as an opening order (buy = long)
      expect(formatOrderLabel(order)).toBe('Market long');
    });
  });
