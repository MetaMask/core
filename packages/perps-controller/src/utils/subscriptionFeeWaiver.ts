...
import { HYPERLIQUID_SCALE_CLOID_MARKER } from './hyperLiquidAdapter.js';
...
/**
 * Whether a client order ID is one this package generated and may re-stamp.
 *
 * Only the Scale ladder's own ids qualify. A caller-supplied
 * `OrderParams.clientOrderId` is the caller's reconciliation key — rewriting a
 * byte of it would submit an id they never chose and can no longer match a fill
 * against — so it is never eligible, and neither is anything malformed.
 *
 * @param clientOrderId - A venue client order ID, or nothing.
 * @returns True when the id was generated by this package.
 */
function isMarkableGeneratedCloid(
  clientOrderId: string | null | undefined,
): boolean {
  const normalized = clientOrderId?.toLowerCase();
  return Boolean(
 ...
 * - **An id this package generated** (a Scale rung) — its own leading marker and
 *   its trailing bytes are preserved, and only the flag byte is set. The Scale
...
export function markSubscriptionCloid(params: {
  ...
  let body: string;
  if (clientOrderId !== undefined && !isMarkableGeneratedCloid(clientOrderId)) {
    // A caller's id, or something malformed. Either way it is not ours to
    // rewrite: hand it back exactly as supplied and forgo attribution.
