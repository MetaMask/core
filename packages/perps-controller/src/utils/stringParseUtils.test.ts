...
describe('parseCommaSeparatedString', () => {
  it('parses comma-separated values', () => {
    expect(parseCommaSeparatedString('xyz:*,abc:TSLA')).toStrictEqual([
      'xyz:*',
      'abc:TSLA',
    ]);
  });
  ...
  it('trims whitespace from values', () => {
    expect(parseCommaSeparatedString('  xyz  , abc  ')).toStrictEqual([
      'xyz',
      'abc',
    ]);
  });
  ...
  it('filters out empty values', () => {
    expect(parseCommaSeparatedString('xyz,,abc')).toStrictEqual(['xyz', 'abc']);
  });

  it('returns empty array for empty string', () => {
    expect(parseCommaSeparatedString('')).toStrictEqual([]);
  });
  ...
  it('handles single value', () => {
    expect(parseCommaSeparatedString('xyz:*')).toStrictEqual(['xyz:*']);
  });
  ...
  it('handles mixed patterns', () => {
    expect(parseCommaSeparatedString('BTC, xyz:TSLA, abc:*, ETH')).toStrictEqual(
      ['BTC', 'xyz:TSLA', 'abc:*', 'ETH'],
    );
  });
