...
describe('idUtils', () => {
  describe('generatePerpsId', () => {
    ...
    it('generates unique IDs with prefix', () => {
      ...
      expect(id1).toMatch(
        /^deposit-[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu,
      );
      expect(id2).toMatch(
        /^deposit-[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu,
      );
    ...
    it('generates different IDs for different prefixes', () => {
      ...
      expect(depositId).toMatch(/^deposit-/u);
      expect(withdrawalId).toMatch(/^withdrawal-/u);
  ...
  describe('generateWithdrawalId', () => {
    it('generates withdrawal IDs with correct format', () => {
      ...
      expect(id).toMatch(
        /^withdrawal-[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu,
      );
    });
  ...
  describe('generateOrderId', () => {
    it('generates order IDs with correct format', () => {
      ...
      expect(id).toMatch(
        /^order-[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu,
      );
    });
  ...
  describe('generateTransactionId', () => {
    it('generates transaction IDs with correct format', () => {
      ...
      expect(id).toMatch(
        /^transaction-[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu,
      );
    });
