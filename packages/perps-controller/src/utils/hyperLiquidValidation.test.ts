...
import type { GetSupportedPathsParams } from '../types';
import { PERPS_ERROR_CODES } from '../constants/perpsErrorCodes';

jest.mock('@metamask/utils', () => ({
  isValidHexAddress: (address: string) => /^0x[0-9a-fA-F]{40}$/u.test(address),
}));
...
describe('hyperLiquidValidation', () => {
  ...
  describe('validateAssetSupport', () => {
    ...
    it('should reject unsupported asset', () => {
      const assetId =
        'eip155:42161/erc20:0xUNSUPPORTED/default' as CaipAssetId;
      const supportedRoutes = [
        {
          assetId:
            'eip155:42161/erc20:0xaf88d065e77c8cc2239327c5edb3a432268e5831/default' as CaipAssetId,
  ...
  describe('applyPathFilters', () => {
    ...
    it('should return all assets when no params provided', () => {
      ...
      expect(result).toStrictEqual(mockAssets);
    });
    ...
    it('should handle unknown symbol', () => {
      ...
      expect(result).toStrictEqual(mockAssets); // Should not filter if symbol not found
    });
  });
