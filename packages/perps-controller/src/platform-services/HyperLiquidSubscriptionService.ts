/* eslint-disable no-restricted-syntax, @typescript-eslint/prefer-nullish-coalescing, promise/always-return, promise/no-nesting, @typescript-eslint/no-floating-promises, jsdoc/require-param-description, jsdoc/require-returns, no-plusplus, no-negated-condition, consistent-return, @typescript-eslint/explicit-function-return-type, promise/param-names */
// TODO: Fix ESLint errors in this file - tracked for Phase 4 cleanup
...
export class HyperLiquidSubscriptionService {
  ...
  private readonly assetCtxsSubscriptionPromises = new Map<
    string,
    Promise<void>
  >(); // Track in-progress subscriptions
  ...
  private extractTPSLFromOrders(
    ...
    orders.forEach((order) => {
      ...
      const matchPositionToTpsl = (pos: Position) => {
      ...
      const matchPositionToCoin = (pos: Position) => pos.symbol === order.coin;
          ...
          .webData2({ user: userAddress }, (data: WebData2WsEvent) => {
            try {
              ...
              const {
                tpslMap,
                tpslCountMap,
                processedOrders: processedOrdersData,
              ...
              if (oiCapsHash !== this.cachedOICapsHash) {
                ...
                this.oiCapSubscribers.forEach((oiCallback) =>
                  oiCallback(oiCaps),
                );
              }
              ...
              if (ordersHash !== this.cachedOrdersHash) {
                ...
                this.orderSubscribers.forEach((ordCallback) =>
                  ordCallback(processedOrdersData),
                );
              }
            ...
            try {
              ...
              data.perpDexStates.forEach((dexState, index) => {
                ...
                if (currentDexNameInLoop) {
                  allOICaps.push(
                    ...oiCaps.map(
                      (symbolName) => `${currentDexNameInLoop}:${symbolName}`,
                    ),
                  );
                } else {
      ...
      const subscription = await subscriptionClient.openOrders(
        ...
        (data: OpenOrdersWsEvent) => {
          ...
          if (
            ...
            const cachedPositionsForDex =
              this.dexPositionsCache.get(cacheKey) || [];
            ...
            } = this.extractTPSLFromOrders(data.orders, cachedPositionsForDex);
    ...
    if (totalSubscribers <= 0) {
      ...
      if (this.webData3Subscriptions.size > 0) {
        this.webData3Subscriptions.forEach((subscription, dexNameKey) => {
          subscription.unsubscribe().catch((unsubError: Error) => {
            this.deps.logger.error(
              ensureError(unsubError),
              this.getErrorContext(
   ...
   */
  private createSubscription<TCallback>(
  ...
  private async ensureAssetCtxsSubscription(dex: string): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to create assetCtxs subscription for DEX: ${dex}`,
      );
    }
  }
        ...
        .assetCtxs(subscriptionParams, (data: AssetCtxsWsEvent) => {
          ...
          metaForCallback.universe.forEach((asset, index) => {
            ...
            if (ctx && 'funding' in ctx) {
              ...
              const priceValue =
                ctx.midPx?.toString() || ctx.markPx?.toString();
              if (priceValue) {
  ...
  public subscribeToOrderBook(params: SubscribeOrderBookParams): () => void {
    ...
    subscriptionClient
      ...
      .then((sub) => {
        if (cancelled) {
          sub.unsubscribe().catch((unsubError: Error) => {
            this.deps.logger.error(
              ensureError(unsubError),
