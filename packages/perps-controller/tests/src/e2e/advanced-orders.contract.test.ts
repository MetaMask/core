...
import {
  ...
  runCase,
  runTypedErrorCase,
} from '../../../e2e/lib/advancedOrders.js';
import type { CaseEvidence } from '../../../e2e/lib/advancedOrders.js';
...
describe('advanced order types contract (e2e matrix)', () => {
  ...
  beforeAll(async () => {
    for (const testCase of cases) {
      ...
      evidence.set(
        testCase.name,
        await runCase({ testCase, runner, mode: 'simulated', symbol: SYMBOL }),
      );
    }
  });
  ...
  it('fails a trigger placement without a trigger price with a typed error', () => {
    const result = runTypedErrorCase();
