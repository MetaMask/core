...
- **BREAKING:** Add strategy placement order types to `OrderType`: `twap`, `scale`, and `chase`, placeable through `placeOrder` alongside the existing `market`, `limit`, and trigger types
  ...
  - A `chase` placement that is still in flight when `disconnect` runs does not register a session. It resolves as a failure with `ORDER_CHASE_ABANDONED` rather than a success, because no strategy is running behind it; the order it rested is reported in `OrderResult.childOrderIds` and stays cancellable through the ordinary single-order path.
