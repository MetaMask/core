...
- **BREAKING:** Add strategy placement order types to `OrderType`: `twap`, `scale`, and `chase`, placeable through `placeOrder` alongside the existing `market`, `limit`, and trigger types
  ...
  - `chase` re-prices by cancelling and re-placing, and sizes each replacement from what the venue still shows resting, so a child that partially filled before it was cancelled is not re-placed at the original size.
