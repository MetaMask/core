...
- Stop `updatePositionTPSL` from submitting a reduce-only trigger larger than the position it protects, which HyperLiquid rejected with `Order 0: Reduce only order would increase position` ([#10037](https://github.com/MetaMask/core/pull/10037))
  - TP/SL sizes are floored onto the asset's size grid instead of rounded half-up. A size on a half-increment (`0.115` at `szDecimals: 2`) was formatted as `0.12` — more than the position — and rejected. This applies to partial (quantity-scoped) TP/SL sizes on both `updatePositionTPSL` and the TP/SL attached to a `placeOrder` call, and to the whole-position size used when position-bound TP/SL is disabled.
  - **A partial TP/SL size smaller than one full size increment is now refused with `ORDER_TPSL_SIZE_INVALID`.** Previously only a size below _half_ an increment was refused: a size in between (for example `0.0005` against `szDecimals: 3`) was rounded up to one increment and submitted. Flooring cannot round up, so it now floors to zero and is refused rather than sent as `'0'`, which the exchange reads as covering the whole position. Clients that submit a partial TP/SL near the asset's minimum size may see this error where the order previously reached the exchange; the smallest accepted size is one full increment. The refusal happens before any signing prompt, builder-fee or referral approval, and before the pre-cancel sweep clears the position's existing triggers, so a rejected update leaves the position's protection untouched.
  - The caller-supplied `position` snapshot is re-validated against the WebSocket position cache before the order is built, as `closePosition` already did. Clients pass a throttled (~1s) snapshot, so a concurrent fill or partial close left its size larger than — or its side opposite to — the real position. The cache read issues no REST request, and a symbol whose DEX has not published on the current connection keeps the caller's snapshot rather than failing the update. Revalidation no longer waits for every expected DEX to publish, so a symbol whose own slice is already live is re-validated during a staggered start or reconnect instead of falling back to the snapshot.
- Stop `closePositions` from submitting a batch close built from a stale position list ([#10037](https://github.com/MetaMask/core/pull/10037))
  - The batch read positions from the aggregate WebSocket cache, which is only rebuilt once every expected DEX has published. A reconnect resets the initialized-DEX set without clearing the caches, so the aggregate could sit frozen at its pre-reconnect contents while the per-DEX slices kept updating. Each position is now refreshed against its own DEX's slice, as `closePosition` already did, so a stale size or side no longer reaches the exchange as a reduce-only order. The slices to consult come from the set of DEXs that have published authoritative positions on the current WebSocket connection, so a position opened on a HIP-3 DEX the frozen aggregate has never seen is still found, while a slice cached before a reconnect is not treated as live until its DEX republishes. Each slice is stamped with the identifier of the WebSocket connection that produced it and is rejected unless that connection is still current, so any reconnect — including one the SDK performs internally without notifying the controller — invalidates it, while a retry on the same connection leaves it intact. Slices are judged individually, so one DEX being live is no longer withheld because another has not published yet. Positions on a DEX that has not published are left as the aggregate reported them. When no aggregate exists and `getPositions()` falls back to REST, that result is returned unchanged rather than overwritten by a WebSocket slice of unknown relative recency.
- Stop `closePosition` from reporting `No position found` for a position that is open ([#10037](https://github.com/MetaMask/core/pull/10037))
  - When no `position` snapshot is supplied, the position was looked up in the aggregate cache described above. In the window after a reconnect, a position opened since the reconnect is absent from that aggregate, so the close failed instead of being submitted. The lookup now consults the per-DEX slices first, including the slice for the symbol's own DEX, so a HIP-3 position missing from the aggregate is still found. A position both the aggregate and its published slice agree is gone still fails closed.
- Stop `editOrder` from rounding a reduce-only size above the size requested ([#10037](https://github.com/MetaMask/core/pull/10037))
  - Edit sizes are formatted with a half-up rounding step, so a reduce-only edit on a half-increment (`0.1155` at `szDecimals: 3`) was submitted as `0.116` and rejected with `Order 0: Reduce only order would increase position`. A reduce-only edit size is now floored onto the asset's size grid. A reduce-only edit smaller than one full increment is refused with `ORDER_SIZE_POSITIVE` rather than submitted as a zero-size modification. Sizes on edits that are not reduce-only keep their existing rounding, since they have no position ceiling to exceed.
- Stop `updateMargin` from reading a stale position side or reporting `No position found` for a live position after a reconnect ([#10037](https://github.com/MetaMask/core/pull/10037))
  - Margin updates now prefer the target DEX's connection-epoch-aware WebSocket slice before deriving `isBuy`. A published slice can replace, add, or remove the aggregate entry; a DEX that has not republished keeps the aggregate fallback. When no usable cache exists, the existing `getPositions()` REST fallback is retained without issuing an additional request or merging an older WebSocket slice over the response.
