...
- **BREAKING:** Resolve HyperLiquid builder-fee applicability through an exhaustive provider-owned order policy and exclude the MetaMask builder fee from TWAP quotes ([#9948](https://github.com/MetaMask/core/pull/9948))
  ...
  - Position TP/SL replacement now places the complete new protection before cancelling the old orders. Partial replacement placement is retracted, and an incomplete cleanup or old-order cancellation returns the replacement order IDs for recovery instead of reporting success.
    - This placement-first ordering supersedes the pre-cancel behavior documented for [#9674](https://github.com/MetaMask/core/pull/9674).
...
- **BREAKING:** Add strategy placement order types to `OrderType`: `twap`, `scale`, and `chase`, placeable through `placeOrder` alongside the existing `market`, `limit`, and trigger types ([#9832](https://github.com/MetaMask/core/pull/9832))
  ...
  - `scale` fans out `OrderParams.scaleNumOrders` limit orders on an inclusive price ladder between `OrderParams.scaleMinPrice` and `OrderParams.scaleMaxPrice`, submitted as a single batch. Sizes are split in whole units of the asset's size grid, so the rungs sum to exactly the submitted size. The batch is not atomic, so a partial placement is retracted. If that cleanup is incomplete, `ORDER_STRATEGY_CANCEL_INCOMPLETE` returns a retryable strategy handle and the remaining `childOrderIds`.
