...
- Stop `closePosition` from submitting reduce-only orders that HyperLiquid rejects with "Reduce only order would increase position" (TAT-3252)
  ...
  - `placeOrder` no longer retries a _full_ close that the exchange rejected for the $10 minimum order value with a 1.5% larger size, since that retry could only fail again as a reduce-only rejection; the minimum-value error is surfaced instead. Partial closes still retry.
