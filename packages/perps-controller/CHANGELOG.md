...
- Stop `closePositions` from submitting a batch close built from a stale position list ([#10037](https://github.com/MetaMask/core/pull/10037))
  - The batch read positions from the aggregate WebSocket cache, which is only rebuilt once every expected DEX has published. A reconnect could leave that aggregate frozen while per-DEX slices moved to a new connection. Each slice is stamped with the connection epoch that produced it; any reconnect, including an SDK-internal reconnect, retires the old slices. The batch uses WebSocket data only when every expected DEX has a current slice, then rebuilds the list from those slices. During cold or staggered initialization it uses a complete REST result instead. REST and WebSocket results are never merged, so an older same-epoch slice cannot overwrite a newer REST response.
- Stop `closePosition` from reporting `No position found` for a position that is open ([#10037](https://github.com/MetaMask/core/pull/10037))
  - When no `position` snapshot is supplied, the position was looked up in the aggregate cache described above. The lookup now uses the symbol's current-connection DEX slice directly, including for HIP-3. If that DEX has not published, it fetches positions over REST instead of trusting the aggregate; a current slice that omits the symbol still proves it is closed and fails before submission.
...
- Stop `updateMargin` from reading a stale position side or reporting `No position found` for a live position after a reconnect ([#10037](https://github.com/MetaMask/core/pull/10037))
  - Margin updates now use the target DEX's current-connection WebSocket slice directly before deriving `isBuy`, including for HIP-3. This preempts the previous REST fallback when that exact slice is live. If the DEX has not republished, the existing REST path runs instead of trusting a stale aggregate. No code path merges REST and WebSocket results or issues more than one position request.
