...
- Stop `closePosition` from submitting reduce-only orders that HyperLiquid rejects with "Reduce only order would increase position" (TAT-3252)
  ...
  - The batch `closePositions` path also rounds each reduce-only size down onto the asset's size grid, instead of letting `formatHyperLiquidSize`'s half-up rounding push it above the position. A position smaller than one size increment is skipped rather than submitted as a zero-size order, and the remaining positions still close; `closePositions` returns an empty result when every position is that small.
  ...
  - `calculateFinalPositionSize` throws `ORDER_SIZE_POSITIVE` when a `reduceOnly` call supplies both a `usdAmount` and a `size` that is not a positive number, instead of capping the USD-derived size to that value and submitting a zero or negative size.
