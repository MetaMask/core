...
- Stop `closePositions` from submitting a batch close built from a stale position list ([#10037](https://github.com/MetaMask/core/pull/10037))
  - The batch read positions from the aggregate WebSocket cache, which is only rebuilt once every expected DEX has published. A reconnect could leave that aggregate frozen while per-DEX slices moved to a new connection. Each slice is stamped with the connection epoch that produced it; any reconnect, including an SDK-internal reconnect, retires the old slices. The batch uses WebSocket data only when every configured DEX has a current slice, then copies one complete snapshot from those slices. During cold or staggered initialization it uses REST instead. A failure from any DEX rejects the whole snapshot with `PROVIDER_NOT_AVAILABLE`; partial results never reach `closeAll`. REST and WebSocket results are never merged.
- Stop `closePosition` from reporting `No position found` for a position that is open ([#10037](https://github.com/MetaMask/core/pull/10037))
  - When no `position` snapshot is supplied, the position was looked up in the aggregate cache described above. The lookup now uses the symbol's current-connection DEX slice directly, including for HIP-3. If that DEX has not published, it queries that DEX through the HTTP info client instead of trusting the aggregate; a current slice or successful HTTP response that omits the symbol proves it is closed. An HTTP failure returns `PROVIDER_NOT_AVAILABLE`, not `No position found`.
...
- Stop `updateMargin` from reading a stale position side or reporting `No position found` for a live position after a reconnect ([#10037](https://github.com/MetaMask/core/pull/10037))
  - Margin updates now use the target DEX's current-connection WebSocket slice directly before deriving `isBuy`, including for HIP-3. If the DEX has not republished, they query that DEX through the HTTP info client instead of trusting a stale aggregate. An HTTP failure returns `PROVIDER_NOT_AVAILABLE`. No path merges REST and WebSocket results or issues more than one position request.
