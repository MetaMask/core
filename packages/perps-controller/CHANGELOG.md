...
- **BREAKING:** Add `positionsSideFilter` and `positionsSortConfig` to `ProLayoutPreferences` (defaults `'all'` and `{ field: 'positionValue', direction: 'desc' }`) so Pro Positions/Orders panel sort and side-filter preferences persist across markets and app restarts via the existing `getProLayoutPreferences()` / `setProLayoutPreferences(patch)` API; export `ProPositionsSideFilter`, `ProPositionsSortField`, `ProPositionsSortConfig`, `ProLayoutPreferencesPatch`, and `mergeProLayoutPreferences` (deep-merges nested `positionsSortConfig` over defaults for predated persisted state) ([#9838](https://github.com/MetaMask/core/pull/9838))
