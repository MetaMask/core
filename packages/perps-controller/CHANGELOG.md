...
- Implement `getMarginModeLock` for Lighter, reporting the mode an open position binds to its market. ([#TBD2](https://github.com/MetaMask/core/pull/TBD2))
- Add optional `supportedMarginModes` to ready order capabilities; HyperLiquid reports `['isolated', 'cross']` for main-DEX markets and `['isolated']` for HIP-3 or isolated-only assets, so clients stop inferring margin-mode support from the provider. ([#TBD2](https://github.com/MetaMask/core/pull/TBD2))
