...
- Stop `updateMargin` from reading a stale position side or reporting `No position found` for a live position after a reconnect ([#10037](https://github.com/MetaMask/core/pull/10037))
  - Margin updates now refresh the aggregate position through the target DEX's connection-epoch-aware WebSocket slice before deriving `isBuy`. A published slice can replace, add, or remove the aggregate entry; a DEX that has not republished keeps the aggregate fallback. The refresh is cache-only and adds no REST request.
