...
> {
   ...
   */
  get gatorPermissionsProviderSnapId(): SnapId { return this.#gatorPermissionsProviderSnapId; }
  ...
  #storedPermissionToPermissionInfo(
    ...
    const { dependencies: _dependencies, to: _to, ...permissionResponse } =
  ...
  #storedPermissionsToPermissionInfoWithMetadata(
    ...
    return storedGatorPermissions.map((storedPermission) => this.#storedPermissionToPermissionInfo(storedPermission));
  }
  ...
  public async addPendingRevocation(
    ...
    const refreshPermissions = (context: string): void => {
      this.fetchAndUpdateGatorPermissions().catch(
        (error) => {
