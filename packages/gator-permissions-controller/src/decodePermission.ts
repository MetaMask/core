import { Caveat, Hex } from '@metamask/delegation-core';
import { getChecksumAddress, hexToBigInt, hexToNumber } from '@metamask/utils';
 ...
 *
 * @param enforcers - List of enforcer contract addresses (hex strings).
 * @param chainId - Chain ID to resolve the enforcer addresses against.
 ...
 *
 * @param chainId - Chain ID used to resolve the enforcer addresses for lookups.
 * @param caveats - Caveats decoded from the permission context.
 * @param permissionType - The previously identified permission type.
...
export const getPermissionDataAndExpiry = ({
  ...
  switch (permissionType) {
    case 'erc20-token-stream': {
      ...
      const tokenAddress = `0x${tokenAddressRaw}`;
      const initialAmount = hexToBigInt(initialAmountRaw);
      const maxAmount = hexToBigInt(maxAmountRaw);
      const amountPerSecond = hexToBigInt(amountPerSecondRaw);
    ...
    case 'erc20-token-periodic': {
      ...
      const tokenAddress = `0x${tokenAddressRaw}`;
      const periodAmount = hexToBigInt(periodAmountRaw);
    ...
    case 'native-token-stream': {
      ...
      const [initialAmountRaw, maxAmountRaw, amountPerSecondRaw, startTimeRaw] =
      ...
      try {
        ...
        throw new Error(
          `Invalid nativeTokenStream terms: ${(error as Error).message}`,
        );
      }
      break;
    }
    case 'native-token-periodic': {
      ...
      const [periodAmountRaw, periodDurationRaw, startTimeRaw] = splitHex(
      ...
      try {
        ...
        throw new Error(
          `Invalid nativeTokenPeriodic terms: ${(error as Error).message}`,
        );
      }
      break;
    }
 ...
 * @param permissionType - Identified permission type.
 * @param delegator - Address of the account delegating permission.
 * @param delegate - Address that will act under the granted permission.
 * @param expiry - Expiry timestamp (unix seconds) or null if unbounded.
 * @param data - Permission-specific decoded data payload.
 ...
 *
 * Example:
 *  splitHex('0x12345678', [1, 3]) => ['12', '345678']
 *
 * @param value - The 0x-prefixed hex string to split.
 * @param lengths - The lengths of each part, in bytes.
 * @returns An array of hex substrings (without the `0x` prefix), one for each part.
 */
const splitHex = (value: Hex, lengths: number[]) => {
  let start = 2;
  const parts: string[] = [];
