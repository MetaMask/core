 ...
 * - `fees.metaMask`: Relay MetaMask fee plus the ramps `extraFee`.
...
function combineQuotes({
  ...
  const rampsProviderFee = rampsFees.providerFee.plus(rampsFees.networkFee);
  ...
  const metaMaskFee = new BigNumber(relayQuote.fees.metaMask.usd)
    .plus(rampsFees.extraFee)
    .toString(10);
