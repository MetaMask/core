...
describe('HyperLiquid Activation', () => {
  ...
  describe('applyHyperliquidActivationFee', () => {
    ...
    describe('HyperLiquid deposit target (TAT-3400)', () => {
      // Normalized perps deposit: Arbitrum USDC remapped to HyperCore USDC.
      // $20.00 margin at 8 decimals = 2000000000.
      const DEPOSIT_TARGET_AMOUNT_MOCK = '2000000000';
      const DEPOSIT_TARGET_WITH_FEE_MOCK = '2100000000';

      const HYPERLIQUID_DEPOSIT_REQUEST_MOCK: QuoteRequest = {
        from: ADDRESS_MOCK,
        isHyperliquidSource: false,
        sourceBalanceRaw: '50000000',
        sourceChainId: '0x1',
        sourceTokenAddress: '0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48' as Hex,
        sourceTokenAmount: '0',
        targetAmountMinimum: DEPOSIT_TARGET_AMOUNT_MOCK,
        targetChainId: '0x539',
        targetTokenAddress: '0x00000000000000000000000000000000' as Hex,
      };

      const PERPS_DEPOSIT_TRANSACTION_MOCK = {
        type: TransactionType.perpsDepositAndOrder,
      } as TransactionMeta;

      it('increases targetAmountMinimum by the activation fee for an unactivated deposit', async () => {
        // Feature flag may be off (withdrawal default); deposits still apply.
        getConfigMock.mockReturnValue({ enabled: false, amountUsd: 1 });
        fetchMock.mockResolvedValue({
          ok: true,
          json: async () => [],
        ...
        const result = await applyHyperliquidActivationFee(
          HYPERLIQUID_DEPOSIT_REQUEST_MOCK,
          MESSENGER_MOCK,
          PERPS_DEPOSIT_TRANSACTION_MOCK,
        );

        expect(result.targetAmountMinimum).toBe(DEPOSIT_TARGET_WITH_FEE_MOCK);
        expect(result.hyperliquidActivationFeeUsd).toBe('1');
        expect(result.sourceTokenAmount).toBe(
          HYPERLIQUID_DEPOSIT_REQUEST_MOCK.sourceTokenAmount,
        );
      });

      it('does not change the target when the deposit account is already activated', async () => {
        getConfigMock.mockReturnValue({ enabled: false, amountUsd: 1 });
        fetchMock.mockResolvedValue({
          ok: true,
          json: async () => [outboundSend()],
        ...
        expect(result).toStrictEqual(HYPERLIQUID_DEPOSIT_REQUEST_MOCK);
      });

      it('does not change a non-perps quote targeting HyperCore', async () => {
        getConfigMock.mockReturnValue({ enabled: false, amountUsd: 1 });
      ...
      it('increases the target for perpsDeposit as well as perpsDepositAndOrder', async () => {
        getConfigMock.mockReturnValue({ enabled: false, amountUsd: 1 });
        fetchMock.mockResolvedValue({
          ok: true,
          json: async () => [],
      ...
      it('uses a custom fee amount from the feature flag for deposits', async () => {
        getConfigMock.mockReturnValue({ enabled: false, amountUsd: 2 });
        fetchMock.mockResolvedValue({
          ok: true,
          json: async () => [],
        } as never);
