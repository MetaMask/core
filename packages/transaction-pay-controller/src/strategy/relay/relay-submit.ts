...
export async function submitRelayQuotes(
  ...
  const { quotes, messenger } = request;

  let transactionHash: Hex | undefined;

  for (const quote of quotes) {
    ({ transactionHash } = await executeSingleQuote(quote.original, messenger));
  }
...
async function executeSingleQuote(
  ...
  if (quote.skipTransaction) {
    log('Updating intent complete flag on transaction', transactionMeta.id);
...
async function waitForRelayCompletion(quote: RelayQuote) {
  const url = `${RELAY_URL_BASE}${quote.steps[0].items[0].check.endpoint}`;
