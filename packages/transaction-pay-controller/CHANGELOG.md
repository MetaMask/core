...
- **BREAKING:** Add subsidized fee to Relay quote target amount if `isMaxAmount`.
  - Remove `human` and `raw` from `targetAmount` on `TransactionPayQuote` and `TransactionPayTotals`.
  - Use `amountFormatted` as USD value for Relay quote target amount when target token is a stablecoin.
  - Add MUSD and Hypercore USDC to stablecoins.
