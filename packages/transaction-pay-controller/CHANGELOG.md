...
- Added `Generic` pay strategy that consumes the MetaMask intents API generic `/quote`, `/submit`, and `/status` endpoints, gated by the `payStrategies.generic.enabled` feature flag (default: disabled).
  ...
  - Sends `supportsGasless` on the outgoing generic quote request when the account supports EIP-7702, Relay execute is enabled, and the source chain is EIP-7702-capable, so backend providers can opt into gasless execution paths (e.g. Relay `originGasOverhead`).
