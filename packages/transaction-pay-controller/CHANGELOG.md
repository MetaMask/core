...
- Stop using the removed Transaction Controller sponsorship and external-sign lifecycle properties in Transaction Pay flows ([#10109](https://github.com/MetaMask/core/pull/10109))
  - Same-chain Relay source-fee calculation and submission no longer read or forward `isGasFeeSponsored` and `isExternalSign`; clients now determine sponsorship and local signing through Transaction Controller approval hooks.
