...
- Add optional `getCanonicalId` constructor callback and `FeatureFlagIdType` enum so threshold flags can segment users by canonical ID instead of MetaMetrics ID when configured with `idType: "canonical"` ([#9289](https://github.com/MetaMask/core/pull/9289))
