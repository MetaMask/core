...
- **BREAKING:** Replaced `RecommendedAction` with `RecommendedActionV1` (no `VERIFIED`) and `RecommendedActionV2` (includes `VERIFIED`). `PhishingController:scanUrl` returns `PhishingDetectionScanResult` with `recommendedAction: RecommendedActionV1`; `PhishingController:bulkScanUrls` returns `PhishingDetectionBulkScanResult` rows with `recommendedAction: RecommendedActionV2`. PDS bulk wire rows use v1 actions (`PhishingDetectionBulkScanWireResult`); the client maps them to v2 for bulk results. `PhishingDetectionScanResult` / `PhishingDetectionBulkScanResult` no longer expose `hostname` or `scanLookupKey` (only `recommendedAction` and optional `fetchError`).
...
- **BREAKING:** `PhishingController:bulkScanUrls` POSTs each caller URL string to PDS and expects `results` / `errors` keyed by the same strings (path-aware gateway scanning stays on `PhishingController:scanUrl` only). `getPhishingDetectionBulkScanUrlParam` is used to validate bulk inputs and read hostnames, not to build the POST body.
