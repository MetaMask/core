...
- **BREAKING:** `getPhishingDetectionBulkScanUrlParam` and `PhishingController:bulkScanUrls` use the same path-aware normalization as `getPhishingDetectionScanUrlParam` / `PhishingController:scanUrl` for `PHISHING_DETECTION_PATH_BASED_ROOT_DOMAINS` (hostname + pathname, query/fragment stripped); other hosts use hostname-only in bulk requests and cache keys.
- `PhishingController:scanUrl` and bulk scans share this normalization. Export of the allowlist and `isPhishingDetectionPathBasedHostname` is available from the package entrypoint.
