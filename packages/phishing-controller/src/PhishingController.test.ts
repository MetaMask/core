...
describe('PhishingController', () => {
  ...
  it('should call bulkScanTokens with extracted token addresses from simulation data', async () => {
    const { messenger, globalMessenger } =
      getRestrictedMessengerWithTransactionEvents();
    const controller = new PhishingController({ messenger });

    const bulkScanTokensSpy = jest
      .spyOn(controller, 'bulkScanTokens')
      .mockResolvedValue({});

    controller.start();

    const mockTransaction = createMockTransaction('test-transaction-1', [
    ...
    // Wait for async token scanning to complete
  ...
  it('should deduplicate token addresses and handle case insensitivity', async () => {
    const { messenger, globalMessenger } =
      getRestrictedMessengerWithTransactionEvents();
    const controller = new PhishingController({ messenger });
    ...
    // Create transaction with duplicate addresses in different cases
    const mockTransaction = createMockTransaction(
      'test-transaction-dedup',
      [],
      {
        simulationData: {
          tokenBalanceChanges: [
            createMockTokenBalanceChange(TEST_ADDRESSES.MOCK_TOKEN_1),
            createMockTokenBalanceChange(
              TEST_ADDRESSES.MOCK_TOKEN_1.toUpperCase() as `0x${string}`,
            ), // Same address, uppercase
            createMockTokenBalanceChange(TEST_ADDRESSES.USDC),
            createMockTokenBalanceChange(
              TEST_ADDRESSES.USDC.toLowerCase() as `0x${string}`,
    ...
    // Verify bulkScanTokens was called with deduplicated addresses (only 2 unique tokens)
    expect(bulkScanTokensSpy).toHaveBeenCalledWith({
      chainId: '0x1',
      tokens: [
        TEST_ADDRESSES.MOCK_TOKEN_1.toLowerCase(),
  ...
  it('should only scan tokens when simulation data changes or transaction is new', async () => {
    const { messenger, globalMessenger } =
      getRestrictedMessengerWithTransactionEvents();
    const controller = new PhishingController({ messenger });
    ...
    const baseTransaction = createMockTransaction('test-transaction-1', [
      TEST_ADDRESSES.MOCK_TOKEN_1,
      TEST_ADDRESSES.USDC,
    ]);
    ...
    globalMessenger.publish(
      'TransactionController:stateChange',
      createMockStateChangePayload([baseTransaction]),
      undefined as never,
    );
    ...
    const updatedTransaction = createMockTransaction('test-transaction-1', [
      TEST_ADDRESSES.USDC, // Different token than before
    ]);
    ...
    expect(bulkScanTokensSpy).toHaveBeenCalledTimes(2);

    bulkScanTokensSpy.mockRestore();
    controller.stop();
  });

  it('should handle async errors in bulkScanTokens method', async () => {
    const { messenger, globalMessenger } =
      getRestrictedMessengerWithTransactionEvents();
    const controller = new PhishingController({ messenger });
    ...
    const bulkScanTokensSpy = jest
      .spyOn(controller, 'bulkScanTokens')
      .mockRejectedValue(new Error('Async scan error'));

    controller.start();

    const mockTransaction = createMockTransaction(
      'test-transaction-async-error',
      [TEST_ADDRESSES.MOCK_TOKEN_1],
    );
  ...
  it('should handle synchronous errors in scanTokensFromSimulation method', async () => {
    const { messenger, globalMessenger } =
      getRestrictedMessengerWithTransactionEvents();
    const controller = new PhishingController({ messenger });
    ...
    // Create a transaction with valid basic structure but problematic tokenBalanceChanges
    const problematicTokenChange = {
      standard: 'erc20' as SimulationTokenStandard,
      difference: '0x1' as `0x${string}`,
      previousBalance: '0x0' as `0x${string}`,
      newBalance: '0x1' as `0x${string}`,
      isDecrease: false,
      // Create a getter that throws when the method tries to access the address
      get address() {
        throw new Error('Synchronous error accessing token address');
      },
    };

    const mockTransaction = createMockTransaction(
      'test-transaction-sync-error',
      [],
      {
        simulationData: {
          tokenBalanceChanges: [
            problematicTokenChange as unknown as SimulationTokenBalanceChange,
          ],
        },
      },
    );
  ...
  it('should handle async errors in scanTokensFromSimulation method', async () => {
    const { messenger, globalMessenger } =
      getRestrictedMessengerWithTransactionEvents();
    const controller = new PhishingController({ messenger });
    ...
    // Mock the bulkScanTokens method to reject a promise, which will cause
    // the async error inside #scanTokensFromSimulation
    const bulkScanTokensSpy = jest
      .spyOn(controller, 'bulkScanTokens')
      .mockRejectedValue(new Error('Async error in bulkScanTokens'));

    controller.start();

    const mockTransaction = createMockTransaction(
      'test-transaction-async-scan-error',
      [TEST_ADDRESSES.MOCK_TOKEN_1],
    );
  ...
  it('should handle errors in onTransactionControllerStateChange method', async () => {
    const { messenger, globalMessenger } =
      getRestrictedMessengerWithTransactionEvents();
    const controller = new PhishingController({ messenger });
    ...
    // Create a transaction with malformed data that causes Map.get to throw
    const malformedTransaction = null as unknown as TransactionMeta;

    globalMessenger.publish(
      'TransactionController:stateChange',
      createMockStateChangePayload([malformedTransaction]),
      undefined as never,
    );
  ...
  it('should not scan tokens when the chainId is empty', async () => {
    const { messenger, globalMessenger } =
      getRestrictedMessengerWithTransactionEvents();
    const controller = new PhishingController({ messenger });
    ...
    const testTransactions = [
      createMockTransaction('test-empty-chain', [TEST_ADDRESSES.MOCK_TOKEN_1], {
    ...
    globalMessenger.publish(
      'TransactionController:stateChange',
      createMockStateChangePayload(testTransactions),
      undefined as never,
    );
  ...
  it('should filter out invalid tokens but scan valid ones', async () => {
    const { messenger, globalMessenger } =
      getRestrictedMessengerWithTransactionEvents();
    const controller = new PhishingController({ messenger });
    ...
    const mockTransaction = createMockTransaction('test-mixed-validity', [], {
      simulationData: {
        tokenBalanceChanges: [
          createMockTokenBalanceChange('' as unknown as `0x${string}`), // Invalid: empty string
          createMockTokenBalanceChange(TEST_ADDRESSES.MOCK_TOKEN_1), // Valid
          createMockTokenBalanceChange(null as unknown as `0x${string}`), // Invalid: null
          createMockTokenBalanceChange(TEST_ADDRESSES.USDC), // Valid: USDC
          createMockTokenBalanceChange(123 as unknown as `0x${string}`), // Invalid: number
        ],
      },
    });
