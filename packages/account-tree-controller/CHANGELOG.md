...
- The `Multichain Account Syncing - Full` trace is now only emitted when a backup-and-sync run actually mutates local or remote state (rolled-back local changes do not count), including runs that fail after doing durable work, and the span is backdated to preserve the real sync duration ([#9343](https://github.com/MetaMask/core/pull/9343))
