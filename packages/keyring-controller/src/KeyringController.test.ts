...
describe('KeyringController', () => {
  ...
  describe('Ledger Keyring', () => {
    ...
    it('should forget Ledger Keyring', async () => {
      await withController(async ({ controller }) => {
        await controller.createNewVaultAndKeychain(password);
        const localLedgerKeyring = await controller.getLedgerKeyring();
        ...
        await controller.forgetLedger();
        expect(controller.state.keyrings[1].accounts).toHaveLength(0);
    ...
    it('should update the state when unlocking the default account on ledger', async () => {
      ...
      await withController(async ({ controller }) => {
        ...
        // Start of test
        const initialAccounts = await controller.getAccounts();
        const defaultHDAccount = initialAccounts[0];

        // now adding the ledger account
        const ledgerAccount = await controller.unlockLedgerDefaultAccount();
        ...
        expect(
          controller.state.keyrings[1].accounts.map((address) =>
            address.toLowerCase(),
          ),
