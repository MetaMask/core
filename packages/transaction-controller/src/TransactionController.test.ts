...
describe('TransactionController', () => {
  ...
  beforeEach(() => {
    ...
    EthQueryMock.mockReturnValue(buildMockEthQuery());
  ...
  describe('addTransaction', () => {
    ...
    it('publishes the transactionNewSwapApproval event after a swap approval transaction is updated with information about the swap', async () => {
      const { controller, messenger } = setupController({
        network: {
          ...MOCK_NETWORK,
          state: {
    ...
    it('publishes the transactionNewSwap event after a swap transaction is updated with information about the swap', async () => {
      const { controller, messenger } = setupController({
        network: {
          ...MOCK_NETWORK,
          state: {
  ...
  describe('confirmExternalTransaction', () => {
    ...
    it("doesn't mark transaction as dropped if local transaction with same nonce and chainId has status of failed", async () => {
      const { controller } = setupController();
      const externalTransactionId = '1';
      const externalTransactionHash = '0x1';
      const externalTransactionToConfirm = {
  ...
  describe('initTrackingMap', () => {
    ...
    it('should handle removals from the networkController registry', (done) => {
      ...
      messenger.publish('NetworkController:stateChange', MOCK_NETWORK.state, [
        {
          op: 'add',
          path: ['networkConfigurations'],
        },
      ]);
...
function setupController({
  ...
  // TODO: Need to subscribe to something and return
  // {}, [
  //    {
  //      op: 'add',
  ...
  const networkClientConfigurationsById = {
    mainnet: {
      chainId: ChainId.mainnet,
    },
    sepolia: {
  ...
  const mockGetNetworkClientById = jest
    ...
    .mockImplementation(
      (
        networkClientId:
          | keyof typeof networkClientConfigurationsById
          | 'global',
      ) => {
        switch (networkClientId) {
          ...
          case 'mainnet':
            return {
              configuration: networkClientConfigurationsById[networkClientId],
          ...
          case 'global':
            return {
              configuration: {
                chainId: network.state.providerConfig.chainId,
              },
              blockTracker: network.blockTracker,
              provider: network.provider,
            };
          default:
            throw new Error(
              `Invalid network client id ${String(networkClientId)}`,
            );
        }
      },
    );
  ...
  const mockFindNetworkClientIdByChainId = jest
    ...
    .mockImplementation((chainId) => {
      if (chainId !== network.state.providerConfig.chainId) {
        throw new Error("Couldn't find networkClientId for chainId");
      }
      return 'global';
    });

  const mockGetNetworkClientRegistry: NetworkController['getNetworkClientRegistry'] =
    () => {
      return ['mainnet', 'sepolia', 'goerli', 'customNetworkClientId-1'].reduce<
        ReturnType<NetworkController['getNetworkClientRegistry']>
      >((obj, networkClientId) => {
        return { ...obj, [networkClientId]: {} } as ReturnType<
          NetworkController['getNetworkClientRegistry']
        >;
      }, {} as ReturnType<NetworkController['getNetworkClientRegistry']>);
    };

  const mockGetNetworkClientIdForDomain: SelectedNetworkController['getNetworkClientIdForDomain'] =
    () => 'global';
  ...
  const { messenger: givenRestrictedMessenger, ...otherOptions } = {
    ...
    getCurrentNetworkEIP1559Compatibility: async () => false,
    getNetworkClientIdForDomain: mockGetNetworkClientIdForDomain,
