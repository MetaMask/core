...
describe('TransactionController', () => {
  ...
  describe('addTransaction', () => {
    ...
    describe('with sponsored approval hooks', () => {
      it('defaults to local signing without sponsorship when hooks are omitted', async () => {
        const { controller } = setupController({
          messengerOptions: {
            addTransactionApprovalRequest: {
              state: 'approved',
      ...
      it('does not refresh sponsorship availability when simulation is disabled', async () => {
        ...
        expect(isSponsoredHook).toHaveBeenCalledWith({
          transactionMeta: expect.not.objectContaining({
            isGasFeeSponsoredAvailable: expect.anything(),
          }),
        });
      });

      it('calls isSponsored hook with current sponsorship availability before reserving a nonce', async () => {
        ...
        const isSponsoredHook = jest
          .fn()
          .mockImplementation(
            async ({ transactionMeta }): Promise<{ isSponsored: boolean }> => {
      ...
      it('preserves deprecated sponsorship metadata without using it as lifecycle policy', async () => {
        const { controller } = setupController({
          messengerOptions: {
            addTransactionApprovalRequest: {
              state: 'approved',
