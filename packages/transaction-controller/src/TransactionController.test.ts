...
describe('TransactionController', () => {
  ...
  describe('GasFeePoller', () => {
    let gasFeePollerHub: EventEmitter;
    const GAS_FEE_POLLER_UPDATE_EVENT = 'transaction-updated';
    ...
    beforeEach(() => {
      gasFeePollerClassMock.mockImplementation(() => {
        gasFeePollerHub = new EventEmitter();
        gasFeePollerMock = {
          hub: gasFeePollerHub,
        } as unknown as jest.Mocked<GasFeePoller>;
        return gasFeePollerMock;
    ...
    it('updates gas fee estimates', async () => {
      const { controller } = setupController({
        options: {
          state: {
            transactions: [TRANSACTION_META_MOCK] as TransactionMeta[],
          },
        },
      });
      ...
      expect(controller.state.transactions[0].gasFeeEstimates).toBe(
    ...
    it('updates gasFeeEstimatesLoaded', async () => {
      const { controller } = setupController({
        options: {
          state: { transactions: [TRANSACTION_META_MOCK] as TransactionMeta[] },
        },
      });
      ...
      expect(controller.state.transactions[0].gasFeeEstimatesLoaded).toBe(true);
      ...
      expect(controller.state.transactions[0].gasFeeEstimatesLoaded).toBe(
        false,
      );
    });
    ...
    it('updates layer1GasFee', async () => {
      ...
      expect(controller.state.transactions[0].layer1GasFee).toBe(
        layer1GasFeeMock,
      );
    });
    ...
    describe('updates txParam gas values', () => {
      ...
      it('with gas price gas fee estimates', () => {
        ...
        gasFeePollerHub.emit(GAS_FEE_POLLER_UPDATE_EVENT, {
          ...GAS_FEE_POLLER_UPDATE_EVENT_PAYLOAD,
          gasFeeEstimates: GAS_PRICE_GAS_FEE_ESTIMATES_MOCK,
        });
