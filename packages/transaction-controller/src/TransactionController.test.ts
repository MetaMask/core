...
describe('TransactionController', () => {
  ...
  beforeEach(() => {
    ...
    signMock = jest
      .fn()
      .mockImplementation(async (transaction) => transaction.toJSON());
  ...
  describe('addTransaction', () => {
    ...
    it('sets txParams.type to 0x2 if not defined in given txParams', async () => {
      ...
      expect(controller.state.transactions[0].txParams.type).toBe(
  ...
  describe('speedUpTransaction', () => {
    ...
    it('creates additional transaction with increased gas', async () => {
      ...
      const { transactions } = controller.state;
      expect(transactions).toHaveLength(2);
      expect(transactions[1].txParams.gasPrice).toBe('0x5916a6d6');
    });
    ...
    it('verifies s,r and v values are correctly populated if values are zero', async () => {
      ...
      expect(speedUpTransaction.v).toBe('0x1b');
    });
  ...
  describe('approveTransactionsWithSameNonce', () => {
    ...
    it('signs transactions and return raw transactions', async () => {
      signMock.mockImplementation(async (transaction) =>
        Promise.resolve(transaction.toJSON()),
      );
  ...
  describe('updateCustodialTransaction', () => {
    ...
    it.each([
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      let actualValue: any = updatedTransaction;

      for (const key of pathParts) {
        // Type assertion needed since we're accessing dynamic properties
        actualValue = actualValue[key as keyof typeof actualValue];
      }
  ...
  describe('abortTransactionSigning', () => {
    ...
    it('sets status to failed if transaction being signed', async () => {
      ...
      rootMessenger.registerActionHandler(
        'KeyringController:signTransaction',
        () => createDeferredPromise<never>().promise,
      );
  ...
  describe('beginAtomicBatchUpdate', () => {
    const requiredAssets = [
      {
        address: ACCOUNT_2_MOCK,
        amount: '0x64' as Hex,
    ...
    function setupAtomicBatchController(
      transactionOverrides: Partial<TransactionMeta> = {},
    ): ReturnType<typeof setupController> {
            ...
            transactions: [
              {
                ...
                status: TransactionStatus.unapproved,
                nestedTransactions: [
                  { to: ACCOUNT_2_MOCK, data: '0x1234' },
                  { to: ACCOUNT_2_MOCK, data: '0x4567' },
                ],
    ...
    it('publishes all nested updates and required assets synchronously in one revision', async () => {
      const { controller } = setupAtomicBatchController();

      const result = controller.beginAtomicBatchUpdate({
        transactionId: TRANSACTION_META_MOCK.id,
        requiredAssets,
        nestedTransactionUpdates: [
          { transactionIndex: 0, transactionData: '0xAAAA' },
          { transactionIndex: 1, transactionData: '0xBBBB' },
        ],
      });
      ...
      expect(
        controller.state.transactions[0].nestedTransactions?.map(
          ({ data }) => data,
        ),
      ).toStrictEqual(['0xAAAA', '0xBBBB']);
      expect(controller.state.transactions[0].txParams.data).toContain('aaaa');
      expect(controller.state.transactions[0].txParams.data).toContain('bbbb');

      expect(await result.preparation).toMatchObject({
        revision: 1,
        status: 'prepared',
      });
      expect(updateGasMock).toHaveBeenCalledTimes(1);
    ...
    it.each([
      {
        expectedError: 'at least one nested transaction update',
        name: 'an empty patch',
        updates: [],
      },
      {
        expectedError: 'Duplicate nested transaction index - 0',
        name: 'duplicate indexes',
        updates: [
          { transactionIndex: 0, transactionData: '0xAAAA' as Hex },
          { transactionIndex: 0, transactionData: '0xBBBB' as Hex },
        ],
      },
      {
        expectedError: 'Nested transaction not found with index - 2',
        name: 'an invalid index',
        updates: [{ transactionIndex: 2, transactionData: '0xAAAA' as Hex }],
      },
    ])(
      'rejects $name without publishing state',
      ({ expectedError, updates }) => {
        const { controller } = setupAtomicBatchController();
        const stateBeforeUpdate = controller.state;

        expect(() =>
          controller.beginAtomicBatchUpdate({
            transactionId: TRANSACTION_META_MOCK.id,
            requiredAssets,
            nestedTransactionUpdates: updates,
          }),
        ).toThrow(expectedError);
        expect(controller.state).toBe(stateBeforeUpdate);
        expect(updateGasMock).not.toHaveBeenCalled();
      },
    );

    it('clears stale revision-bound preparation metadata while preserving non-gas reverts', async () => {
      ...
      updateGasMock.mockImplementationOnce(async () => {
        await gasPreparation.promise;
      });
      const { controller } = setupAtomicBatchController({
        gasLimitNoBuffer: '0x111',
        gasUsed: '0x222',
        revert: {
    ...
    it('ignores simulation results started before the current revision', async () => {
      ...
      const { controller } = setupAtomicBatchController();
      getBalanceChangesMock.mockReturnValueOnce(staleSimulation.promise);
      shouldResimulateMock.mockReturnValueOnce({
        blockTime: 123,
        resimulate: true,
      });
      ...
      const update = controller.beginAtomicBatchUpdate({
        transactionId: TRANSACTION_META_MOCK.id,
        requiredAssets,
        nestedTransactionUpdates: [
          { transactionIndex: 0, transactionData: '0xAAAA' },
      ...
      await update.preparation;

      staleSimulation.resolve({
        simulationData: {
          ...SIMULATION_DATA_RESULT_MOCK,
      ...
      expect(controller.state.transactions[0].transactionRevision).toBe(1);
    });

    it('increments revisions and prevents stale gas from overwriting a newer revision', async () => {
      const firstGas = createDeferredPromise<void>();
      const { controller } = setupAtomicBatchController();

      updateGasMock
        .mockImplementationOnce(async ({ txMeta }) => {
          await firstGas.promise;
          txMeta.txParams.gas = '0x111';
        })
        .mockImplementationOnce(async ({ txMeta }) => {
          txMeta.txParams.gas = '0x222';
        });

      const first = controller.beginAtomicBatchUpdate({
        transactionId: TRANSACTION_META_MOCK.id,
        requiredAssets,
        nestedTransactionUpdates: [
          { transactionIndex: 0, transactionData: '0xAAAA' },
      ...
      const second = controller.beginAtomicBatchUpdate({
        transactionId: TRANSACTION_META_MOCK.id,
        requiredAssets,
        nestedTransactionUpdates: [
          { transactionIndex: 0, transactionData: '0xBBBB' },
        ],
      });

      expect(await second.preparation).toMatchObject({
        revision: 2,
        status: 'prepared',
      });
      firstGas.resolve();
      expect(await first.preparation).toMatchObject({
        revision: 1,
        status: 'superseded',
      });

      expect(controller.state.transactions[0].transactionRevision).toBe(2);
      expect(controller.state.transactions[0].txParams.gas).toBe('0x222');
      expect(
        controller.state.transactions[0].nestedTransactions?.[0].data,
      ).toBe('0xBBBB');
    });
  });
  ...
  describe('messenger actions', () => {
    ...
    describe('TransactionController:transactionApproved event', () => {
      it('does not publish transactionApproved event when keyring throws during signing', async () => {
        ...
        rootMessenger.registerActionHandler(
          'KeyringController:signTransaction',
          jest.fn().mockRejectedValue(signingError),
        );
