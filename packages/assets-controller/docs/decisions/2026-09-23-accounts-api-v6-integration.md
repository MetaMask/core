...
### Stale balance scenarios (`full`)

| #   | What happened                                                                           | What we write                                                                | What the user sees                                                         |
| --- | --------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| 1   | Snap is unreachable (throws or returns nothing)                                         | Nothing for that account                                                     | Last amounts stay on screen                                                |
| 2   | Snap answers but skips a native coin, a token the user added, or a default such as mUSD | Copy the last known amount into the snapshot                                 | That token stays listed at the last successful read                        |
| 3   | Same skip, but we have never stored an amount for it                                    | A placeholder zero so the asset appears in the list                          | The asset shows at `0`. This is **not** stale — there is no earlier amount |
| 4   | One RPC `balanceOf` / decimals call fails; the rest of the chain succeeds               | Keep the last amount for that token only; write fresh amounts for the others | The unread token stays listed; everything else updates                     |
| 5   | Same RPC failure, but the token was already `0` and is not one we track on purpose      | Leave it out of the snapshot                                                 | The unused detected token disappears from the list                         |
| 6   | Every RPC call on the chain fails                                                       | Nothing for that chain; retry via RPC fallback                               | Last amounts on that chain stay until fallback                             |
| 7   | Accounts API could not fully answer a chain                                             | Mark the chain failed and strip its balances; retry via RPC fallback         | Last amounts on that chain stay until fallback                             |

### Stale balance scenarios (`merge`)

| #   | What happened                                                                               | What we write                               | What the user sees                                                 |
| --- | ------------------------------------------------------------------------------------------- | ------------------------------------------- | ------------------------------------------------------------------ |
| 8   | Account Activity event after a tx omits a token that actually changed                       | Overlay only the tokens named in the event  | Omitted token stays at the last poll                               |
| 9   | Account Activity row has no `asset`, `postBalance`, or `decimals`; or the event is empty    | Nothing for those tokens; no write if empty | Last amounts stay                                                  |
| 10  | Account Activity sends an amount that is wrong                                              | Overlay overwrites that token               | Wrong number until the next Accounts API `full` poll               |
| 11  | Snap live event names some holdings but skips another token that changed                    | Overlay only the tokens in the event        | Omitted token stays until the next Snap `full` poll                |
| 12  | Staking source updates one vault and not another, or a token poll never includes staked ETH | Do not overwrite the omitted vault          | Staked ETH stays at the last staking read until that vault is sent |
