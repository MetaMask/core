...
export type AccountsApiDataSourceConfig = {
  ...
  /**
   * Feature flag gating the Accounts API balances endpoint version
   * (default: () => false, i.e. v5).
   *
   * When it returns true, balances are fetched from the v6 endpoint;
   * otherwise the legacy v5 endpoint is used. Kept as a getter so the value
   * can be driven by a remote feature flag and toggled at runtime, allowing a
   * revert to v5 without re-instantiating the data source.
   */
...
> {
   ...
   * getter so it can be flipped at runtime (e.g. to revert v6 -> v5).
   */
  ...
  constructor(options: AccountsApiDataSourceOptions) {
    ...
    this.#useBalanceV6 = options.useBalanceV6 ?? ((): boolean => false);
