...
import * as evmRpcServicesMocks from './evm-rpc-services';
import {
  RpcDataSource,
  caipChainIdToHex,
  createRpcDataSource,
...
describe('RpcDataSource', () => {
  beforeEach(() => {
    ...
    (evmRpcServicesMocks.MulticallClient as jest.Mock).mockImplementation(
      (providerGetter: (hexChainId: string) => unknown) => {
        currentMulticallProviderGetter = providerGetter;
        return mockMulticallClient;
      },
    );
    (evmRpcServicesMocks.BalanceFetcher as jest.Mock).mockImplementation(
      () => currentMockBalanceFetcher,
    );
    (evmRpcServicesMocks.TokenDetector as jest.Mock).mockImplementation(
