...
export type BalancePollingInput = {
   ...
   * `assetsBalance` entries (Accounts API v6 asset-scoped RPC poll). Must be
   ...
   * subscription on the Accounts API v5 path.
   */
...
export class BalanceFetcher extends StaticIntervalPollingControllerOnly<BalancePollingInput>() {
  ...
  #getAssetsToFetch(
    ...
    // Assets tracked with a balance entry — only when not in
    ...
    // User-added custom assets — on the v5 path RPC is the sole balance
    // fetcher for these, so they must be polled even if they have no balance
    // entry yet. On the v6 path they are usually already in `assetsBalance`.
  ...
  async #fetchBalances(
    ...
    const assets = this.#getAssetsToFetch(
      chainId,
      accountId,
      assetIds,
      customAssetsOnly,
