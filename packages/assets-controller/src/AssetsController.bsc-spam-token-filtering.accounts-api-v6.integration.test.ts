...
// CDOGE/BNB. These snapshots pin the whole persisted state, so any
// unexpected asset sneaking in, any captured asset dropping out, or any
// field-level drift (fast lane or slow lane) shows up as a reviewable diff.
// Volatile price timestamps are zeroed so the records are deterministic.
// Inline snapshots (per the repo lint rule banning `toMatchSnapshot`)
// cannot be written from inside `describe.each`, so the passes are spelled
// out explicitly.
describe('AssetsController (Accounts API v6): BNB Chain spam token (CDOGE) golden records', () => {
  afterEach(() => {
    cleanAll();
  });

  it('first pass over a fresh wallet - full state as a golden record', async () => {
    const { state } = await fetchWallet();

    expect(withZeroedTimestamps(state)).toMatchInlineSnapshot(`
      {
        "assetPreferences": {},
        "assetsBalance": {
          "b1b2c3d4-e5f6-4a7b-8c9d-0e1f2a3b4c5d": {
            "eip155:56/erc20:0x003d87d02A2A01E9E8a20f507C83E15DD83A33d1": {
              "amount": "910.2040000000002",
            },
  ...
  it('second pass over the wallet the first pass left behind - full state as a golden record', async () => {
    const firstPass = (await fetchWallet()).state;
    cleanAll();
    ...
    expect(withZeroedTimestamps(secondPass.state)).toMatchInlineSnapshot(`
      {
        "assetPreferences": {},
        "assetsBalance": {
          "b1b2c3d4-e5f6-4a7b-8c9d-0e1f2a3b4c5d": {
