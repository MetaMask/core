...
describe('AssetsController', () => {
  ...
  describe('keyring lifecycle', () => {
    ...
    it('removes spam assets from state on keyring unlock', async () => {
      const MOCK_SPAM_ASSET =
        'eip155:1/erc20:0x1111111111111111111111111111111111111111' as Caip19AssetId;
      const fetchV3Assets = jest.fn().mockResolvedValue([
        {
          assetId: MOCK_ASSET_ID,
          name: 'USD Coin',
          symbol: 'USDC',
      ...
      const fetchV1SuggestedOccurrenceFloors = jest
        .fn()
        .mockResolvedValue({ '1': 3 });

      const queryApiClient = {
        ...createMockQueryApiClient(),
        tokens: { fetchV3Assets },
      ...
      await withController(
        ...
        async ({ controller, messenger }) => {
          ...
          expect(controller.state.assetsInfo[MOCK_SPAM_ASSET]).toBeUndefined();
          expect(
            controller.state.assetsBalance[MOCK_ACCOUNT_ID]?.[MOCK_SPAM_ASSET],
          ).toBeUndefined();
          expect(fetchV1SuggestedOccurrenceFloors).toHaveBeenCalled();
          expect(fetchV3Assets).toHaveBeenCalled();
        },
      );
    });

    it('does not run spam cleanup when basic functionality is disabled', async () => {
      const MOCK_SPAM_ASSET =
        'eip155:1/erc20:0x1111111111111111111111111111111111111111' as Caip19AssetId;
      const fetchV3Assets = jest.fn().mockResolvedValue([]);
      const fetchV1SuggestedOccurrenceFloors = jest
      ...
      await withController(
        ...
        async ({ controller, messenger }) => {
          ...
          expect(controller.state.assetsInfo[MOCK_SPAM_ASSET]).toBeDefined();
          expect(fetchV1SuggestedOccurrenceFloors).not.toHaveBeenCalled();
          expect(fetchV3Assets).not.toHaveBeenCalled();
        },
      );
    });
