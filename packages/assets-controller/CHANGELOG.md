...
- `RpcDataSource` and `StakedBalanceDataSource` now use their own messenger types (`RpcDataSourceMessenger`, `StakedBalanceDataSourceMessenger`) instead of `AssetsControllerMessenger`. `AssetsController` creates child messengers and delegates the required actions and events to each data source ([#7951](https://github.com/MetaMask/core/pull/7951))
