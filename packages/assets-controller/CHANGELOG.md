...
- `AssetsController` subscribes to `AccountActivityService:balanceUpdated` so unified `assetsBalance` state updates from real-time websocket notifications when `AccountActivityService` owns the server subscription ([#9273](https://github.com/MetaMask/core/pull/9273))
