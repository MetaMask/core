...
- On the Accounts API v6 path, the Accounts API, Snap, RPC fallback, and RPC polling share the same native/pin/default/hidden visibility policy; hidden assets are not fetched but their existing balances remain in state ([#9651](https://github.com/MetaMask/core/pull/9651))
...
- Apply an Accounts API v6 `full` update as the complete asset list for the chains it covers, only carrying over assets the source was not asked about (hidden assets and staking positions); natives, pins, and default tracked assets such as mUSD survive a force refresh because v6 requests them as `includeAssetIds` ([#9651](https://github.com/MetaMask/core/pull/9651))
