...
- `SnapDataSource.fetch` and subscription initial fetches stamp `updateMode: 'full'` on the Accounts API v6 path and add missing visible assets at zero, so non-EVM pins survive authoritative snapshots; Snap balance events remain merge deltas
