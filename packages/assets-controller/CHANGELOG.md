...
- Add `AccountsApiDataSourceConfig`, `PriceDataSourceConfig`, and `StakedBalanceDataSourceConfig` types; add `accountsApiDataSourceConfig`, `priceDataSourceConfig`, and `stakedBalanceDataSourceConfig` options to `AssetsControllerOptions` for per-data-source configuration. `accountsApiDataSourceConfig` supports pollInterval, tokenDetectionEnabled, etc.; when `tokenDetectionEnabled` is false, `AccountsApiDataSource` only returns balances for tokens already in state and does not add new tokens. `priceDataSourceConfig.pollInterval` is used when subscribing to price updates (spot-price polling). `stakedBalanceDataSourceConfig` supports `enabled` and `pollInterval` for the staked balance data source.
  ([#7936](https://github.com/MetaMask/core/pull/7936))
