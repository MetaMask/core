...
- **BREAKING:** Remove unused `KycCustomerIdentity`.
- **BREAKING:** Remove `sessionStatusPollIntervalMs` and `userStatusPollIntervalMs` from `KycControllerOptions`, and stop automatic session-status and user-status polling. `startSumSub` fetches UKYC session status once after the SDK completes; `getSessionStatus` remains one-shot.
