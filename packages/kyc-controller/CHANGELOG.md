...
- **BREAKING:** Require `sessionClientPublicKey` (unpadded base64url X25519 public key) on `KycService.createUkycSession` (`POST /sessions`). The controller generates the per-session keypair before creating the session and uses the private half to wrap authorizations.
