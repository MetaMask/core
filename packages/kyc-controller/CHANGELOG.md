...
- Add a consents-path KYC flow on `KycController` for non-MoonPay vendors (currently `iron`): empty-shell customer → disclaimers → consents → SumSub, skipping MoonPay Check/Auth frames. `initialize({ vendor })` and `createVendorCustomer({ vendor, email })` drive the path; `acceptTermsAndStartSession` still takes optional `sumsubTncSigned` / `idosTncSigned` (ignored for MoonPay). ([#9908](https://github.com/MetaMask/core/pull/9908))
