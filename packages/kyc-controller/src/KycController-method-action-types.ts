 ...
 * 2. verifies the `encryptionDataKey` schema's `jwtChain` against the Fractal
 * JWKS and the `ukycCapabilityToken` schema's `jwtChain` against the idOS
 * relay JWKS, then confirms each attested session server public key;
