import type { IndicatorColors, TVActiveChart } from '../../../core/types.js';
import {
...
describe('resolveStudyPreset', () => {
  it('returns MACD preset with all four colours', () => {
    ...
    expect(preset.inputs).toStrictEqual({ in_0: 12, in_1: 26, in_2: 9 });
  ...
  it('returns RSI preset with default 14 period', () => {
    ...
    expect(preset.inputs).toStrictEqual({ in_0: 14 });
  ...
  it('returns BOL preset', () => {
    ...
    expect(preset.inputs).toStrictEqual({ in_0: 20, in_1: 2 });
  ...
  it('returns MA200 preset without colour override', () => {
    ...
    expect(preset.inputs).toStrictEqual({ length: 200 });
  ...
  it('returns MA variants with the right length and colour', () => {
    ...
    expect(preset.inputs).toStrictEqual({ length: MA_LENGTHS.MA5 });
  ...
  it('returns a generic preset for unknown names', () => {
    ...
    expect(preset.inputs).toStrictEqual({ foo: 1 });
  });
});
