...
export const pluginSuperstructName =
  'plugin-superstruct' satisfies PluginSuperstruct['name'];
...
export const pluginSuperstruct = definePlugin<PluginSuperstruct>(
  (options = {}) => {
    ...
    return {
      ...
      pre: [pluginOasName],
      resolvePath(baseName, pathMode, resolvePathOptions): string {
        ...
        if (
          group &&
          (resolvePathOptions?.group?.path || resolvePathOptions?.group?.tag)
        ) {
