...
`,
  ...
  it('ignores variable declarations that are not MESSENGER_EXPOSED_METHODS', async () => {
    expect.assertions(2);

    await withinParseSourceSandbox(async ({ directoryPath }) => {
      const controllerFile = path.join(directoryPath, 'OtherVarController.ts');
      await fs.promises.writeFile(
        controllerFile,
        `
const SOMETHING_ELSE = ['unused'] as const;
const MESSENGER_EXPOSED_METHODS = ['doStuff'] as const;

class OtherVarController {
  doStuff() {
    return true;
  }
}
`,
  ...
  it('returns null when MESSENGER_EXPOSED_METHODS has no initializer', async () => {
    expect.assertions(1);

    await withinParseSourceSandbox(async ({ directoryPath }) => {
      const controllerFile = path.join(
        directoryPath,
        'NoInitializerController.ts',
      );
      await fs.promises.writeFile(
        controllerFile,
        `
let MESSENGER_EXPOSED_METHODS: readonly string[];

class NoInitializerController {
  doStuff() {
    return true;
  }
}
`,
  ...
  it('extracts JSDoc from the first overload signature of an overloaded method', async () => {
    expect.assertions(3);

    await withinParseSourceSandbox(async ({ directoryPath }) => {
      const controllerFile = path.join(directoryPath, 'OverloadController.ts');
      await fs.promises.writeFile(
        controllerFile,
        `
const MESSENGER_EXPOSED_METHODS = ['doStuff'] as const;
...
class OverloadController {
   ...
   * Overload 2 JSDoc.
   */
  doStuff(arg: number): number;
  doStuff(arg: string | number): string | number {
    return arg;
  }
}
`,
      ...
      expect(result).not.toBeNull();
      expect(result?.methods[0].name).toBe('doStuff');
      expect(result?.methods[0].jsDoc).toContain('Overload 1 JSDoc.');
    });
  });

  it('picks the closest JSDoc when multiple JSDoc blocks precede a method', async () => {
    expect.assertions(3);

    await withinParseSourceSandbox(async ({ directoryPath }) => {
      const controllerFile = path.join(directoryPath, 'MultiDocController.ts');
      await fs.promises.writeFile(
        controllerFile,
        `
const MESSENGER_EXPOSED_METHODS = ['doStuff'] as const;
...
`,
      ...
      expect(result).not.toBeNull();
      expect(result?.methods[0].jsDoc).toContain('Closest JSDoc block.');
      expect(result?.methods[0].jsDoc).not.toContain('Earlier JSDoc block.');
    });
  });

  it('returns null when MESSENGER_EXPOSED_METHODS is not an array literal', async () => {
    expect.assertions(1);

    await withinParseSourceSandbox(async ({ directoryPath }) => {
      const controllerFile = path.join(directoryPath, 'NotArrayController.ts');
      await fs.promises.writeFile(
        controllerFile,
        `
const MESSENGER_EXPOSED_METHODS = getMethods();

class NotArrayController {
  doStuff() {
    return true;
  }
}
