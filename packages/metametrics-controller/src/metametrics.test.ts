...
const segment = createSegmentMock(2);
...
/**
 * Returns a mock preferences store object with the specified current locale.
 *
 * @param options - Optional parameters for the mock preferences store.
 * @param options.currentLocale - The current locale for the preferences store. Defaults to LOCALE.
 * @returns A mock preferences store object with the following properties:
 *   - getState: A sinon stub that returns the current state of the preferences store.
 *   - updateState: A function that updates the state of the preferences store and triggers the subscribe callback.
 *   - subscribe: A sinon stub that can be used to listen for state changes in the preferences store.
 */
function getMockPreferencesStore({ currentLocale = LOCALE } = {}) {
  ...
  return {
    getState: stub().returns(preferencesStore),
    updateState,
    subscribe,
  };
}
...
/**
 * Creates a new instance of the MetaMetricsController class with the provided options.
 *
 * @param options - The options for creating the MetaMetricsController instance.
 * @param [options.participateInMetaMetrics] - Whether the user should participate in MetaMetrics.
 * @param [options.metaMetricsId] - The MetaMetrics ID to use.
 * @param [options.preferencesStore] - The preferences store to use.
 * @param [options.getCurrentChainId] - The function to get the current chain ID.
 * @param [options.onNetworkDidChange] - The function to handle network changes.
 * @param [options.segmentInstance] - The Segment instance to use.
 * @returns The newly created MetaMetricsController instance.
 */
...
describe('MetaMetricsController', function () {
  ...
  beforeEach(function () {
    ...
    stub(Utils, 'generateRandomId').returns('DUMMY_RANDOM_ID');
  });

  describe('constructor', function () {
    it('should properly initialize', function () {
      const mock = sinonMock(segment);
      mock
        .expects('track')
        .once()
        .withArgs({
  ...
  describe('submitEvent', function () {
    ...
    it('should immediately flush queue if flushImmediately set to true', async function () {
      const metaMetricsController = getMetaMetricsController();
      const flushStub = stub(segment, 'flush');
    ...
    it('should track sensitiveProperties in a separate, anonymous event', function () {
      const metaMetricsController = getMetaMetricsController();
      const spy = sinonSpy(segment, 'track');
      metaMetricsController.submitEvent({
        event: 'Fake Event',
        category: 'Unit Test',
        sensitiveProperties: { foo: 'bar' },
  ...
  describe('submitting segmentApiCalls to segment SDK', function () {
    ...
    it('should remove event from store when callback is invoked', function () {
      ...
      stub(segmentInstance, 'track').callsFake(stubFn);
      stub(segmentInstance, 'page').callsFake(stubFn);
