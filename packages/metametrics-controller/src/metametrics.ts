...
import { isManifestV3 } from './modules';
...
const defaultCaptureException = (err: Error) => {
...
const buildUniqueMessageId = (args: any) => {
...
/**
 * @typedef {import('./constants/metametrics').MetaMetricsContext} MetaMetricsContext
 * @typedef {import('./constants/metametrics').MetaMetricsEventPayload} MetaMetricsEventPayload
 * @typedef {import('./constants/metametrics').MetaMetricsEventOptions} MetaMetricsEventOptions
 * @typedef {import('./constants/metametrics').SegmentEventPayload} SegmentEventPayload
 * @typedef {import('./constants/metametrics').SegmentInterface} SegmentInterface
 * @typedef {import('./constants/metametrics').MetaMetricsPagePayload} MetaMetricsPagePayload
 * @typedef {import('./constants/metametrics').MetaMetricsPageOptions} MetaMetricsPageOptions
 * @typedef {import('./constants/metametrics').MetaMetricsEventFragment} MetaMetricsEventFragment
 * @typedef {import('./constants/metametrics').MetaMetricsTraits} MetaMetricsTraits
 */
...
export default class MetaMetricsController {
  /**
   * Constructs a new instance of the MetaMetricsController class.
   *
   * @param options - The options for creating the controller.
   * @param options.segment - The Segment analytics client.
   * @param options.preferencesStore - The store for user preferences.
   * @param options.onNetworkDidChange - The callback function to be called when the network changes.
   * @param options.getCurrentChainId - The function to get the current chain ID.
   * @param options.version - The version of the application.
   * @param options.environment - The environment the application is running in.
   * @param [options.initState] - The initial state of the controller.
   * @param [options.extension] - The extension the controller is associated with.
   * @param [options.captureException] - The function to capture exceptions.
   */
  constructor({
    segment,
    preferencesStore,
    ...
    this._captureException = (err: Error) => {
   ...
   *
   * @param options - Fragment settings and properties
   ...
   * @param payload - Fragment settings and
   ...
   * @param options
   * @param [options.abandoned] - if true track the failure
   *  event instead of the success event
   * @param [options.page] - page the final event
   *  occurred on. This will override whatever is set on the fragment
   * @param [options.referrer] - Dapp that
   ...
   *
   * @param participateInMetaMetrics - Whether or not the user wants
   ...
   * @param [options] - options for handling the page
   *  view
   * @param payload.environmentType
   * @param payload.page
   * @param payload.referrer
   * @param payload.actionId
   */
  trackPage(
   ...
   * @param [options] - options for routing and
   *  handling the event
   * @returns
   */
