...
- Tag hardware, imported-private-key, and non-mnemonic Snap accounts with a `source` on `AccountWithScopes` when submitting profile metrics
...
- Canonicalize account addresses when they are enqueued rather than only when a proof of ownership is signed, so accounts without a proof (hardware, imported, non-mnemonic Snap) are also submitted in canonical form
