...
- Add `NetworkController:rpcEndpointAvailable` messenger event ([#????](https://github.com/MetaMask/core/pull/????))
...
- **BREAKING:** Split up and update payload data for `NetworkController:rpcEndpoint{Degraded,Unavailable}` ([#????](https://github.com/MetaMask/core/pull/????))
...
- **BREAKING:** Rename and update payload data for `NetworkController:rpcEndpointRequestRetried` ([#????](https://github.com/MetaMask/core/pull/????))
...
- **BREAKING:** Update `AbstractRpcService`/`RpcServiceRequestable` to remove `{ isolated: true }` from the `onBreak` event data type ([#????](https://github.com/MetaMask/core/pull/????))
