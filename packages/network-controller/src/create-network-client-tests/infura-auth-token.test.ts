...
describe('createNetworkClient - Infura auth token', () => {
  ...
  it('presents the token when no `fetch` is given', async () => {
    await withMockedCommunications(
      {
        providerType: 'infura',
  ...
  it('keeps the headers from a `getRequestHeaders` given via `getRpcServiceOptions`', async () => {
    await withMockedCommunications(
      { providerType: 'infura' },
      async (comms) => {
        comms.mockNextBlockTrackerRequest({ blockNumber: '0x1' });
        ...
        const headersByRequest: (Record<string, string> | undefined)[] = [];
        // The RPC service always sends plain-object headers.
        const recordingFetch: typeof fetch = async (input, init) => {
          headersByRequest.push(init?.headers as Record<string, string>);
          return await fetch(input, init);
        };
        ...
        expect(result).toBe('0xabc');
        expect(headersByRequest.length).toBeGreaterThan(0);
        expect(
          headersByRequest.filter(
            (headers) =>
