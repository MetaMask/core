...
// Payload properties are an analytics wire format: snake_case names are the
// shared vocabulary emitted by every client.
...
/**
 * A redirect destination: a screen, a bottom sheet, or a client URL. The
 * `string & Record<never, never>` arm admits client-defined URL constants
 * without collapsing the union (which would lose enum autocomplete).
 */
...
export type MoneyRedirectEventProperties = {
  redirect_target: MoneyRedirectTarget;
};
 ...
 * the client resolves into both. The two forms are mutually exclusive so a
 * label and its key can never contradict.
 */
...
/**
 * Input accepted by `trackButtonClicked`. Differs from the payload type only
 * in that the TEXT variant additionally accepts `label_key`.
 */
