...
describe('areMoneyAccountVaultConfigsEqual', () => {
  const config = VALID_CONFIG as MoneyAccountVaultConfig;

  it('treats identical configs as equal', () => {
    expect(areMoneyAccountVaultConfigsEqual(config, { ...config })).toBe(true);
  });

  for (const key of ['chainId', ...ADDRESS_KEYS] as const) {
    it(`treats configs differing in ${key} as unequal`, () => {
      expect(
        areMoneyAccountVaultConfigsEqual(config, {
          ...config,
