...
describe('parseAccountsApiActivity', () => {
  ...
  it('parses card and cashback rows from the same response, in input order', () => {
    ...
    // Assert
    expect(result.map((a) => a.kind)).toStrictEqual(['card', 'cashback']);
  });
  ...
  it('returns an empty array when there is no data', () => {
    expect(parseAccountsApiActivity({}, MONEY_ADDRESS)).toStrictEqual([]);
  });
  ...
  it('drops rows that carry no valueTransfers field at all', () => {
    const withoutTransfers = ({
      valueTransfers: _ignored,
      ...rest
    }: Record<string, unknown>): Record<string, unknown> => rest;

    expect(
      parseAccountsApiActivity(
        {
          data: [
            withoutTransfers(cardPaymentRow),
            withoutTransfers(unclassifiedCashbackRow),
          ],
        } as Parameters<typeof parseAccountsApiActivity>[0],
        MONEY_ADDRESS,
      ),
    ).toStrictEqual([]);
  });

  it('ignores a multisend row with no target address at all', () => {
    const response = {
      data: [{ ...unclassifiedCashbackRow, to: undefined }],
    } as unknown as Parameters<typeof parseAccountsApiActivity>[0];

    expect(parseAccountsApiActivity(response, MONEY_ADDRESS)).toStrictEqual([]);
  });

  it('ignores an errored multisend row even when the cashback heuristics match', () => {
    expect(
      parseAccountsApiActivity(
        { data: [{ ...unclassifiedCashbackRow, isError: true }] },
        MONEY_ADDRESS,
      ),
    ).toStrictEqual([]);
  });

  it('ignores an unclassified multisend row on a non-accepted chain', () => {
    expect(
      parseAccountsApiActivity(
        { data: [{ ...unclassifiedCashbackRow, chainId: 1 }] },
        MONEY_ADDRESS,
      ),
    ).toStrictEqual([]);
  });

  it('drops rows with a malformed chainId (non-number, non-integer, negative)', () => {
    // Cast through `unknown`: these payloads deliberately violate the wire
    // types to simulate untrustworthy JSON.
    const response = {
      data: [
        { ...cardPaymentRow, chainId: '143' },
        { ...cardPaymentRow, chainId: 143.5 },
...
describe('dedupeAccountsApiActivity', () => {
  const row = (
    kind: AccountsApiActivity['kind'],
    hash: string,
  ): AccountsApiActivity => ({ kind, hash, time: 0 }) as AccountsApiActivity;

  it('drops a repeated (kind, hash) pair, keeping the first occurrence', () => {
    // An inclusive API cursor can repeat the last row of one page as the
    // first of the next.
    const deduped = dedupeAccountsApiActivity([
      row('card', '0xAAA'),
      row('card', '0xaaa'),
      row('card', '0xbbb'),
    ]);

    expect(deduped.map((entry) => entry.hash)).toStrictEqual([
      '0xAAA',
      '0xbbb',
    ]);
  });

  it('keeps rows sharing a hash but differing in kind (spend + its cashback)', () => {
    const deduped = dedupeAccountsApiActivity([
      row('card', '0xaaa'),
      row('cashback', '0xaaa'),
    ]);
  ...
  it('returns an empty array for an empty input', () => {
    expect(dedupeAccountsApiActivity([])).toStrictEqual([]);
  });
});
