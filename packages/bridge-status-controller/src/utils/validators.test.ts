...
describe('validators', () => {
  ...
  describe('intentStatusValidator', () => {
    it('returns true for a valid intent status response', () => {
      expect(
        validateIntentStatusResponse({
          id: 'intent-id',
          ...
          txHash: '0x1234',
          metadata: {
            txHashes: ['0xabcd', '0xdef0'],
          },
        }),
      ).toBe(true);
    });

    it('returns true when metadata.txHashes is a string', () => {
      expect(
        validateIntentStatusResponse({
          id: 'intent-id',
    ...
    it('returns false for invalid intent status response', () => {
      expect(
        validateIntentStatusResponse({
          id: 'intent-id',
          status: 'not-a-status',
  ...
  describe('quoteStatusUpdateResponseValidator', () => {
    it.each([
      {
        description: 'base error type (no current/new status)',
        input: {
          statusCode: 409,
      ...
      {
        description: 'on-chain mismatch type with current/new status',
        input: {
          statusCode: 400,
          message: 'Status mismatch',
          type: QuoteStatusUpdateErrorType.QuoteStatusOnChainMismatch,
          currentStatus: QuoteStatusUpdateStatus.Served,
          newStatus: QuoteStatusUpdateStatus.Submitted,
        },
      },
      {
        description: 'on-chain mismatch type with finalized statuses',
        input: {
          statusCode: 400,
          message: 'Invalid status transition',
          type: QuoteStatusUpdateErrorType.InvalidStatusTransaction,
          currentStatus: QuoteStatusUpdateStatus.FinalizedFailed,
          newStatus: QuoteStatusUpdateStatus.FinalizedSuccess,
        },
      },
    ])('does not throw for valid response: $description', ({ input }) => {
      expect(() => validateQuoteStatusUpdateResponse(input)).not.toThrow();
    });
    ...
    it.each([
      ...
      {
        description: 'mismatch type missing current/new status fields',
        input: {
          statusCode: 400,
          message: 'Status mismatch',
          type: QuoteStatusUpdateErrorType.QuoteStatusOnChainMismatch,
      ...
      {
        description: 'mismatch type with invalid status values',
        input: {
          statusCode: 400,
          message: 'Status mismatch',
          type: QuoteStatusUpdateErrorType.QuoteStatusOnChainMismatch,
          currentStatus: 'bad-status',
    ...
    ])('throws for invalid response: $description', ({ input }) => {
      // eslint-disable-next-line jest/require-to-throw-message
      expect(() => validateQuoteStatusUpdateResponse(input)).toThrow();
    });
  });
});
