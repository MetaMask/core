...
> {
   ...
   * @param options.globalId - An optional identifier, generated by the UI query
   * client, that correlates this service-side mutation with the specific UI
   * mutation that triggered it. When provided, it is stored on the mutation's
   * `meta` so it is included in dehydrated `:cacheUpdated` payloads.
  ...
  > & {
    ...
    >(this.#queryClient, {
      ...options,
      // A `globalId` correlates this service-side mutation with the specific UI
      // mutation that triggered it. We store it on `meta` so it rides along in
      // dehydrated `:cacheUpdated` payloads, where the UI side uses it to find
      // and update the exact mutation it created.
      ...(globalId !== undefined && {
