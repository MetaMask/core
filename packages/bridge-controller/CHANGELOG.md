...
- Added `getLocation()` method to `BridgeController` for reading the current swap/bridge entry point
- Added `Unknown` value to `MetaMetricsSwapsEventSource` enum for unattributed swap and bridge flows
...
- **BREAKING:** Default `location` fallback for Unified SwapBridge events now uses `Unknown` instead of `Main View` when no entry point is set
