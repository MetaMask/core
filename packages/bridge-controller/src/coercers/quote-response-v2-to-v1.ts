...
const QuoteV1FromV2 = coerce(QuoteSchema, QuoteSchemaV2, (value) => {
  ...
  const { chainId: srcChainIdInCaip } = parseCaipAssetType(src.asset.assetId);
  const { chainId: destChainIdInCaip } = parseCaipAssetType(dest.asset.assetId);
  ...
  return {
    ...
    srcChainId,
    destChainId,
    srcAsset: toBridgeAssetV1(src.asset),
    ...
    feeData: {
      [FeeType.METABRIDGE]: {
        ...metabridgeFeeData,
        asset: toBridgeAssetV1(metabridgeFeeData.asset),
      },
