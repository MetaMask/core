...
import { SortOrder, RequestStatus, ChainId, NonEvmFees } from './types';
...
import { BridgeAssetV2, BridgeAssetV2FromV1 } from './validators/bridge-asset';
import { validateQuoteResponseV1 } from './validators/quote-response';
import {
  QuoteResponse,
  validateQuoteResponse,
} from './validators/quote-response-v2';
import {
  DeepPartial,
  mergeQuoteMetadata,
  toQuoteResponseV2,
} from './validators/quote-response-v2-migration';
  ...
  describe('selectBridgeQuotes', () => {
    const getMockState = (
      ...
      const mockQuoteV2 = [
        ...
        .map((quote) => toQuoteResponseV2(quote))
        .map((quote) => ({
    ...
    describe('returns swap metadata', () => {
      const getMockSwapState = (
        ...
        return {
          ...mockState,
          quotes: [quoteResponse as never].map((quote) =>
      ...
      it('when gas is included and is taken from src token', () => {
        ...
        const expectedQuoteMetadata = {
          ...
          cost: {
            usd: '1936.533955083193541376',
            valueInCurrency: '1656.9444638622599996415',
          },
          gasFee: {
            effective: {
              amount: '0.000008087',
          ...
          sentAmount: {
            amount: '3.018116598427479256',
            usd: '1947.04737997753541763072',
            valueInCurrency: '1665.93999999999999972688',
          },
          swapRate: '0.0054023554269661573',
          toTokenAmount: {
            amount: '0.016304938584731331',
            usd: '10.51864197978187625472',
            valueInCurrency: '9.00000000000000008538',
    ...
    it('should handle Solana quotes', () => {
      ...
      const expectedQuoteMetadata = {
        ...
        minToTokenAmount: {
          amount: '1.8',
          usd: '180000',
          valueInCurrency: '90009',
        },
        quote: solanaState.quotes[1].quote,
        sentAmount: {
          amount: '1000000000',
          usd: '10000000000',
          valueInCurrency: '500000000',
        },
        nonEvmFeesInNative: '5000',
        swapRate: '0.0000000021',
        toTokenAmount: {
          amount: '2.1',
          usd: '210000',
          valueInCurrency: '105010.5',
        },
        totalMaxNetworkFee: {
          amount: '5000',
          usd: '50000',
      ...
      validateQuoteResponse(solanaQuote);

      expect(result.recommendedQuote).toStrictEqual(
        mergeQuoteMetadata(solanaQuote, expectedQuoteMetadata),
      );
    });
  });
