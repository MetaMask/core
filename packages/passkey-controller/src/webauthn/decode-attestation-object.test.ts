...
describe('decodeAttestationObject', () => {
  it('decodes base64url-encoded indirect attestationObject', () => {
    ...
    expect(decoded.get('attStmt')).toStrictEqual(new Map());
    expect(Boolean(decoded.get('authData'))).toBe(true);
  });

  it('decodes base64url-encoded direct attestationObject', () => {
    const decoded = decodeAttestationObject(
      base64URLToBytes(
    ...
    expect(decoded.get('fmt')).toBe('fido-u2f');
    expect(Boolean(decoded.get('attStmt').get('sig'))).toBe(true);
    expect(Boolean(decoded.get('attStmt').get('x5c'))).toBe(true);
    expect(Boolean(decoded.get('authData'))).toBe(true);
  });
});
