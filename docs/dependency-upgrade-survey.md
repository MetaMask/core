## What dropping the allowlist would unleash

A survey of all 219 external dependencies in core, ahead of deciding whether to open the `@metamask/*` allowlist. The short version is that two thirds of the backlog is mechanical and the repair workflow already handles it, so the real question is the 53 majors, and only ten of those look genuinely awkward.

| | Count |
| --- | --- |
| External dependencies | 219 |
| Already current | 59 |
| **Patch or minor behind** | **100** (35 patch, 65 minor) |
| Major behind | 54 (53 real, 1 alias artefact) |
| Unresolvable | 6 |

That middle band is the important number. Patch and minor bumps are exactly what the repair workflow was built for: it aligns ranges, resolves and deduplicates the lockfile, and writes changelog entries without anyone reading the diff. A hundred of those arriving over a few weeks is volume, not risk.
...
**Published surface.** A dependency in `dependencies` or `peerDependencies` reaches consumers; a devDependency does not. 36 of the majors are published, 18 are dev only.

**Module format at the target.** If the new major drops its `require` condition, Jest cannot `requireActual` it without `transformIgnorePatterns` work. That is what stopped `uuid` at 11 rather than 14. 8 majors are ESM only at latest.

**Blast radius.** How many manifests declare it. Nine touch ten or more, where a mistake means a wide revert.

**Client precedent.** Whether metamask-extension or metamask-mobile already resolves that major in its lockfile. If they do, the ecosystem risk is largely proven. 14 qualify.

### Where the score is blind

It does not see a second copy of a dependency arriving through another package. `@metamask/snaps-sdk` scores in the middle, but #10025 is stuck precisely because `@metamask/eth-snap-keyring` pins its own nested copy and the two `SnapId` branded types refuse to unify. Treat the ranking as triage, not a verdict.
...
| Package | Current → latest | Majors | Manifests | Signals | Tier |
| --- | --- | --- | --- | --- | --- |
...
<details>
<summary>All 53 majors, hardest first</summary>
...
| `@lavamoat/allow-scripts` | `^3.0.4` → `5.1.0` | +2 | 10 | none | Moderate |
...
| `@types/readable-stream` | `^2.3.0` → `4.0.24` | +2 | 3 | none | Moderate |
...
| `@lavamoat/preinstall-always-fail` | `^2.1.0` → `3.0.0` | +1 | 8 | none | Easy |
| `jest-when` | `^3.7.0` → `4.0.3` | +1 | 3 | none | Easy |
| `@types/better-sqlite3` | `^7.6.13` → `9.6.0` | +2 | 1 | none | Easy |
...
| `comment-json` | `^4.5.1` → `5.0.0` | +1 | 1 | none | Easy |
| `eslint` | `^9.39.1` → `10.10.0` | +1 | 1 | none | Easy |
| `eslint-import-resolver-typescript` | `^3.6.3` → `4.4.5` | +1 | 1 | none | Easy |
| `eslint-plugin-n` | `^17.10.3` → `18.3.0` | +1 | 1 | none | Easy |
...
Worth noting that four of the ten hard ones were already sitting in the Dependabot backlog as stuck pull requests, so opening the allowlist does not create the problem. It just makes the rest of it visible.
