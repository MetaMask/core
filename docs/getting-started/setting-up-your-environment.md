...
3. Run `yarn install` to install dependencies and run any required post-install scripts. To install or refresh [MetaMask agent skills](../processes/agent-skills.md), run `yarn skills`.
   - Note: This command emits `YN0060`/`YN0002`/`YN0086` peer-dependency warnings. While not ideal, these are somewhat expected and should be ignored.
