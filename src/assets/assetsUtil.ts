 ...
 */
 export function compareCollectiblesMetadata(newCollectibleMetadata: CollectibleMetadata, collectible: Collectible) {
     ...
     const differentValues = keys.reduce((value, key) => {
        if (newCollectibleMetadata[key] && (newCollectibleMetadata[key] !== collectible[key])) {
          return value + 1
        }
        return value
      }, 0)
