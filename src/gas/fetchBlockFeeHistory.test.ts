...
describe('fetchBlockFeeHistory', () => {
  let queryMock: WhenMock;
  const ethQuery = { eth: 'query' };

  beforeEach(() => {
    queryMock = when(mockedQuery).mockImplementation((...args: any[]) => {
      return Promise.reject(
        new Error(
          'None of the calls you mocked for this function matched the actual calls. ' +
            `Given args: ${JSON.stringify(args)}`,
        ),
      );
    });
  });
