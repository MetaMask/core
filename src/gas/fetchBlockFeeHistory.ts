 ...
 * @param blockCount - The number of blocks requested.
 * @param endBlockNumber - The number of the block at the end of the requested range.
 */
type RequestChunkSpecifier = {
...
export default async function fetchBlockFeeHistory<Percentile extends number>({
  ...
  const historicalBlockChunks = await Promise.all(
  ...
  return historicalBlockChunks.reduce(
    (array, historicalBlocks) => [...array, ...historicalBlocks],
    [] as Block<Percentile>[],
  );
}
