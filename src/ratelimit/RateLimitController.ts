...
> = {
  requests: Record<string, Record<keyof RateLimitedApis, number>>;
};
...
> {
  ...
  constructor({
    ...
    state?: RateLimitState<RateLimitedApis>;
  ...
  _isRateLimited(api: keyof RateLimitedApis, origin: string) {
    return this.state.requests[origin][api] >= this.rateLimitCount;
  }
  ...
  _recordRequest(api: keyof RateLimitedApis, origin: string) {
    this.update((state) => {
      (state as any).requests[origin][api] =
        ((state as any).requests[origin][api] ?? 0) + 1;
  ...
  _resetRequestCount(api: keyof RateLimitedApis, origin: string) {
    this.update((state) => {
      (state as any).requests[origin][api] = 0;
    });
  }
}
