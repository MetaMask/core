...
- Shared entry ([#9960](${REPO_URL}/pull/9960))`,
  ...
  it('keeps pre-rename release links correct for a renamed package', async () => {
    const oldTagPrefix = 'old-name@';
    const newTagPrefix = '@metamask/new-name@';
...
- Added after the rename ([#2](${REPO_URL}/pull/2))

## [1.0.0]

### Added

- Added before the rename ([#1](${REPO_URL}/pull/1))

[Unreleased]: ${REPO_URL}/compare/${newTagPrefix}2.0.0...HEAD
[2.0.0]: ${REPO_URL}/compare/${oldTagPrefix}1.0.0...${newTagPrefix}2.0.0
[1.0.0]: ${REPO_URL}/releases/tag/${oldTagPrefix}1.0.0
`;

    const { content: merged } = await mergeChangelogs({
      ourContent: content,
      theirContent: content,
      repoUrl: REPO_URL,
      tagPrefix: newTagPrefix,
      packageRename: {
        tagPrefixBeforeRename: oldTagPrefix,
        versionBeforeRename: '1.0.0',
      },
    });
...
describe('resolvePackageMetadata', () => {
  ...
  it('includes packageRename for a package known to have been renamed', async () => {
    jest.spyOn(fs, 'readFile').mockResolvedValue(
      JSON.stringify({
        name: '@metamask/json-rpc-engine',
        repository: { url: 'https://github.com/MetaMask/core.git' },
      }),
    );
    ...
    expect(result.packageRename).toStrictEqual({
