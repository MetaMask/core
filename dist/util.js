 ...
 */
function validateCollectibleToWatch(collectible) {
    const { address, tokenId } = collectible;
    if (!address || !tokenId) {
        throw eth_rpc_errors_1.ethErrors.rpc.invalidParams(`Must specify address and tokenId.`);
    }
    if (typeof address !== 'string') {
        throw eth_rpc_errors_1.ethErrors.rpc.invalidParams(`Invalid address: not a string.`);
    }
    if (typeof tokenId !== 'string') {
        throw eth_rpc_errors_1.ethErrors.rpc.invalidParams(`Invalid tokenId: not a string.`);
    }
    if (!isValidHexAddress(address)) {
        throw eth_rpc_errors_1.ethErrors.rpc.invalidParams(`Invalid address "${address}".`);
    }
}
exports.validateCollectibleToWatch = validateCollectibleToWatch;
/**
