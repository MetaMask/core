...
var SuggestedCollectibleStatus;
(function (SuggestedCollectibleStatus) {
    SuggestedCollectibleStatus["accepted"] = "accepted";
    SuggestedCollectibleStatus["failed"] = "failed";
    SuggestedCollectibleStatus["pending"] = "pending";
    SuggestedCollectibleStatus["rejected"] = "rejected";
})(SuggestedCollectibleStatus || (SuggestedCollectibleStatus = {}));
...
class CollectiblesController extends BaseController_1.BaseController {
    ...
    failSuggestedCollectible(suggestedCollectibleMeta, error) {
        const failedSuggestedCollectibleMeta = Object.assign(Object.assign({}, suggestedCollectibleMeta), { status: SuggestedCollectibleStatus.failed, error });
        this.hub.emit(`${suggestedCollectibleMeta.id}:finished`, failedSuggestedCollectibleMeta);
    }
    /**
     ...
     */
    watchCollectible(collectible) {
        return __awaiter(this, void 0, void 0, function* () {
            const suggestedCollectibleMeta = {
                collectible,
                id: this._generateRandomId(),
                status: SuggestedCollectibleStatus.pending,
                time: Date.now(),
            };
            try {
                (0, util_1.validateCollectibleToWatch)(collectible);
                const { selectedAddress } = this.config;
                if (!(yield this.isCollectibleOwner(selectedAddress, collectible.address, collectible.tokenId))) {
                    throw new Error('This collectible is not owned by the user');
                }
                const collectibleInfo = yield this.getCollectibleInformation(collectible.address, collectible.tokenId);
                suggestedCollectibleMeta.collectible = Object.assign(Object.assign({}, suggestedCollectibleMeta.collectible), collectibleInfo);
            }
            catch (error) {
            ...
            const result = new Promise((resolve, reject) => {
                this.hub.once(`${suggestedCollectibleMeta.id}:finished`, (meta) => {
                    switch (meta.status) {
     ...
     */
    acceptWatchCollectible(suggestedCollectibleID) {
        return __awaiter(this, void 0, void 0, function* () {
            const { suggestedCollectibles } = this.state;
            const index = suggestedCollectibles.findIndex(({ id }) => suggestedCollectibleID === id);
            const suggestedCollectibleMeta = suggestedCollectibles[index];
            try {
                const { address, tokenId } = suggestedCollectibleMeta.collectible;
                yield this.addCollectibleVerifyOwnership(address, tokenId);
            ...
            const newSuggestedCollectibles = suggestedCollectibles.filter(({ id }) => id !== suggestedCollectibleID);
     ...
     * @param suggestedCollectibleID - The ID of the suggestedCollectible to accept.
     */
    rejectWatchCollectible(suggestedCollectibleID) {
        const { suggestedCollectibles } = this.state;
        const index = suggestedCollectibles.findIndex(({ id }) => suggestedCollectibleID === id);
        const suggestedCollectibleMeta = suggestedCollectibles[index];
