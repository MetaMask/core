...
cacheTest(
  'getBlockForNumber for latest then block 0',
  [
    {
...
cacheTest(
  'getBlockForNumber for latest then block 1',
  [
    {
      method: 'eth_getBlockByNumber',
      params: ['latest'],
...
cacheTest(
  'getBlockForNumber for 0 then block 1',
  [
    {
      method: 'eth_getBlockByNumber',
      params: ['0x0'],
...
cacheTest(
  'getStorageAt for same block should cache',
  [
    {
...
cacheTest(
  'getStorageAt for different block should not cache',
  [
    {
      method: 'eth_getStorageAt',
      params: ['0x295a70b2de5e3953354a6a8344e616ed314d7251', '0x0', '0x1234'],
...
async function cacheTest(label, basePayloads, shouldCache) {
  test(`block-cache - ${label}`, async (t) => {
    try {
      ...
      // prepare payloads
      const payload1 = Object.assign(
        {},
        Array.isArray(basePayloads) ? basePayloads[0] : basePayloads,
        { id: 1, jsonrpc: '2.0' },
      );
      const payload2 = Object.assign(
        {},
        Array.isArray(basePayloads) ? basePayloads[1] : basePayloads,
        { id: 2, jsonrpc: '2.0' },
      );
      ...
      t.equal(
        hitCountMiddleware.getHits(payload1.method).length,
        1,
        `${label} - should hit dataProvider`,
      );
      ...
      if (shouldCache) {
        t.equal(
          hitCountMiddleware.getHits(payload2.method).length,
          1,
          `${label} - should NOT hit dataProvider`,
        );
      } else {
        t.equal(
          hitCountMiddleware.getHits(payload2.method).length,
          2,
          `${label} - should again hit dataProvider`,
        );
      }
    } catch (err) {
      t.fail(err);
